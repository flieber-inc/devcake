...
- Storage: `/data/secrets/connections/{scope}-{instance}.json` (scope ∈ pmo|repo|skill) and `/data/secrets/harness/{VAR}.json`, written 0600 (`mkstemp` + `fsync` + `os.replace`). The two-level path means the existing redaction glob (`security._known_values`, `glob("*/*")`) auto-covers every value; each write also `register_runtime_secret`s it. Skill-source read tokens (`token` / `token_ro`) use the `skill` scope (ADR-0016 addendum 2) and ride settings-bundle / profile snapshots with the other connection secrets.
- Config schema **v4**: the `*_env` fields are gone; `RepoInstance.token`/`token_ro`/`reviewer_token`, `PMOInstance.api_key`, and `SkillSource.token`/`token_ro` are read-through properties over the store. `pmos`/`repos`/`skill_sources` relax to 0..N so a truly empty first boot is a defined idle state (GUI-only setup).
