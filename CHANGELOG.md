...
- **Changed — DevCake's own writes invalidate the mirror freshness
  ...
  an app-side merge, or a claims push to a notebook drops that
  repository's freshness so the next dispatch resyncs regardless of the
  window. The window covers passive staleness only, and a Dev's mid-run
  pulls always go to the real forge.
