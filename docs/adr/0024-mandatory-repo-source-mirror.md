...
7. **Own writes invalidate the window.** `sync_max_age_seconds` covers passive staleness only. When DevCake itself changes a repository — a run finishes on its work repository (the Dev may have pushed), the app merges a pull request (the finalize-time auto-merge or the merge sweep), or the claims writer pushes a notebook — that card's mirror freshness is dropped (`RepoCache.invalidate`), so the next dispatch resyncs regardless of the window. The feed memo's own-write rule (ADR-0003 amendment), applied to mirrors: what the app knows it wrote is never served stale. A Dev's working copy is only seeded from the mirror — its origin is the real forge and mid-run pulls go there — so the window only ever delays what a checkout starts from.
