...
- The provisioner is constructed only via `adapters.registry.make_internal_forge`/`make_gitea_adapter`; the naming convention (`internal_repo_name`) lives on `ports/internal_forge.py` so the domain derives it without importing the adapter (F1 boundary).
