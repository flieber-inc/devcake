...
export default function deriveAlerts(health) {
  ...
  for (const card of health.claims_queue_capped || []) {
    alerts.push({
      ...
      body: "New leads are being refused. Run the Memory Curator (Settings → Scheduled Tasks) to review the queue, or raise the cap under Settings → Limits → Counting budgets — old leads are never deleted automatically.",
    });
  }
