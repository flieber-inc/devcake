...
| `harness_template` | a `HARNESSES` key (`claude-code`, `grok-build`, `codex`, `pi`) | **Authoritative** (2026-07-12 rework; H2): the Docker image, credential requirements, and OAuth flow all derive from it via the harness registry (`app/devcake/harness.py`, `08-harness-templates.md` §2/§4). Changing it in the admin panel changes what actually runs. Config validates against the registry — no parallel Literal. |
