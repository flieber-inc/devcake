...
Per-mission repository resolution is **sticky once a writing run exists** (PLAN/EXECUTE/REVIEW): a conflicting marker edit gates with a visible reason. ONBOARD is read-only triage with every work repository mounted, so an ONBOARD run never latches the repository — the first writing step resolves the marker (`domain/repo_routing.py`).
