...
| Repo sourcing (mission-type → repo set) | 3 copies (`needed_for`, `_extra_repos_for`, `_blocker_mount_ok`) kept in sync by comment | one shared predicate (the `at_decomposition_limit` precedent); gate-set == mount-set parametrized test |
