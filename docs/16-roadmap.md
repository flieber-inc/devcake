...
Final state (initial cut): 662 unit tests + admin `check:ui` green; `ci_suite.sh` (pin gate, gitea battery 13/13, dispatch-hello smoke) green on the deployed stack; the live clear-under-load smoke passed. Trailers that landed with the cut or immediately after: **`profiles.mjs` in `check:ui`** (PR #36) and the missions-board 1280 layout fix (PR #37) — full suite **61** browser checks including profiles. Remaining non-gating trailer (⏳): a fresh-`/data` operator-drill re-run.

**Post-tag hygiene (2026-07-19, moves `v0.2`):** compose mounts `./dagu/dags` **read-only** into Dagu (trusted launch code — `14` §5; live-verified: health + hello dispatch; non-fatal `.dag.index` write WARN). Doc drift fixed: multi-connection is in-scope at runtime (`00`, `06`), RO DAG mount is no longer aspirational (`13`, `14`), and the v0.2 final-state numbers above no longer claim profiles deferred.
