...
| **Dev Type** | A named configuration: harness template + model selection + identifying prompt + MCP servers + credentials + concurrency cap. v0 seeds three: **Senior Dev** (Claude Fable / Claude Code), **Main Dev** (Grok 4.5 / Grok Build), and **Junior Dev** (Claude Haiku / Claude Code — the Relations Mapper's default vehicle). The model belongs to the Dev Type; multiple Dev Types may share one harness. |
