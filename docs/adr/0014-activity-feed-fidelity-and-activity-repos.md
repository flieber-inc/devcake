...
- **Security posture (docs/14 §1 · §7 · §10 Zone B):** feed content and full dumps become durably greppable by every future Dev via Gitea instead of living only in wipeable payloads and PMO attachments. Redaction at the feed/finalize choke-points is the only scrubbing — it does **not** cover Dev egress (`14` §7). A human pasting a secret into the PMO feed now lands it in a repo until Clear. Accepted under the single-operator / dedicated-host trust model (`14` §0); not multi-tenant isolation.
