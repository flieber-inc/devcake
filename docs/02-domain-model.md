...
| `mcp_setup_commands` | `list[str]` | Shell commands run by the Dev entrypoint before harness launch (e.g. `claude mcp add …`). Delivered as a top-level runspec key, live-read at `runspec.get` (like the secret half — an edit applies to the next run without redispatch). Failure ⇒ exit code 14. |
| `secret_env` | `list[str]` | **Names** of GUI-stored secrets (`/data/secrets/harness/{VAR}.json`) delivered into the run's env — mission tooling credentials referenced as `$VAR` from `mcp_setup_commands` (e.g. `DD_API_KEY` for `devcake-logs-mcp`). UPPER_SNAKE_CASE, ≤64 chars; `DEVCAKE_*`/`OTEL_*`/`GIT_*`/`PATH`/`HOME` refused (they would shadow the protocol env). Missing value ⇒ warn-and-proceed. Global store: one stored value serves any number of Dev Types. |
