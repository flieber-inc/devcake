...
- **Status:** accepted (2026-08-11); **amended in-body** (numbered
  implementation rulings; ruling 14 deletes the numeric routing budgets —
  dedup + family size are the bounds). Founder + assistant sparring session;
...
The per-cycle sweep and the merge driver re-read a labeled mission's whole feed to recompute board arithmetic (posted − receipted) and the merge-state markers. On a metered vendor (ADR-0040) that read multiplies with board size, so it is memoized per mission (`FeedScanMemo`, `04-orchestrator.md` §1): a scan is reused while the mission's `updated_at` is unchanged, DevCake itself has not written to that feed since (every DevCake-authored comment passes through the feed chokepoint, which invalidates the memo), and — only where the adapter declares that `updated_at` does not move for comments (`PMOCapabilities.updated_at_tracks_comments` false) — the scan is younger than a five-minute safety window that catches a human's comment there. The memo decides **when** the arithmetic is recomputed, never **what** pending is: pending is always posted − receipted from the feed; a truncated full-history scan is never memoized (fail-closed stays fail-closed); and a caller about to **write** on the strength of the scan — the steward's routing apply, the discovery dispatch, and the sweep itself before it drops the label or closes a batch with `to=-` — bypasses the memo and pays the live read. Process-local by construction: a restart or a config reload rescans.

## Addendum — environment findings are not discoveries; the same finding lands once (2026-09-04)

Field observation: a harness limitation ("the Dev cannot read the tracker's approvals") was rediscovered by sibling after sibling, filed as a discovery each time, and routed by the steward to dozens of in-progress recipients per run — feed traffic on a metered board, and noise on every ticket. Two rulings, both product-side:

1. **Environment findings are declined at both ends.** The Dev's discovery instruction says that a limitation of its own run — a credential it lacks, a tracker or API it could not reach, an absent mount — is about the run, not the family's code, and belongs in the summary or hand-off. The steward's binding rules say to decline such findings with reason `environment` and never route them. Judgment stays the budget (ruling 14); these sentences tell it what the family's work is not.
2. **Content dedup at delivery.** Every harvest post and every delivery carries one fingerprint line per finding (`` `devcake:finding:v1 sha=…` ``: twelve hex characters of a SHA-256 over the finding text, case-folded, every run of non-alphanumerics in any script collapsed to one space), unquoted next to the counted marker so the inline-ceiling fallback keeps it. Before delivering, the apply step reads the recipient's fingerprints — earlier deliveries from any source and the recipient's own harvests — and skips a finding already there, auditing `discovery_route_duplicate_content`; the receipt still records the route, so the batch dispositions, and when nothing of a pair was fresh the receipt says so. The pair dedup of Decision 6 bounds the same batch; this bounds the same finding from many sources. Not a budget — the second bound the ADR always named.

## Addendum — one feed-changes read stands in for the per-mission re-reads (2026-09-06)

The memo above re-reads a labeled feed whenever its mission's `updated_at` moved, and on Linear that timestamp moves for every label swap, status change, and relation edit DevCake itself makes — reads that find nothing new — while a threaded reply does not move it at all (verified live), so a human's reply inside a step's thread is visible to the memo only through the witness below. Where the vendor can list a team's feed entries created, modified, or removed since a moment (`PMOCapabilities.feed_delta`; port method `feed_changes_since`, one paged read), the poll spends one such read per cycle and uses it as a **validity witness**: a memoized scan whose mission changed is kept when the witness shows no entry of that feed newer than the scan's newest entry, and dropped when one is. Three rulings keep it honest. **The witness carries no text** — ids and timestamps only — so the two existing scans stay the only derivations of feed state (the memo decides when, never what; a second parser was considered and refused, since the shallow merge scan carries no entry ids to dedup against and the posted-marker list would double-count on overlap re-delivery). **Timestamps are the vendor's**: the watermark starts at the board's newest `updated_at` (and follows it while nothing is memoized), advances to the newest change reported, and is asked with a short overlap; the comparison is change time against the scan's newest entry time, never a local clock. The overlap re-delivers rows, and an edit or a removal carries a change time no entry's creation time reaches, so the reported change time is remembered per mission as the floor of the next scan's newest-entry time — captured before the scan's read, like the generation — and a changed feed is re-read once, not on every cycle. **Doubt costs a read, never a wrong answer**: a page-capped witness proves nothing and drops every memoized scan for that cycle; a refused read (the request budget) applies the per-mission rule unchanged; a permanent failure latches the witness off until the next settings Save or a restart. A witness never refreshes a scan's age: a vendor that keeps the safety rescan keeps it, and the rescan catches what the witness cannot list (a removed comment on a vendor that does not report removals); on Linear an archived comment is listed when the archival moves its `updatedAt`. Forge-issue adapters that expose a repository-wide "comments since" listing declare the capability; the others keep the per-mission read.
...
## Addendum — the discovery package is built to a prompt budget (2026-09-08)

Field receipt: every discovery-steward run on one instance died eight seconds after dispatch for four days, recorded as "dagu run dead (no heartbeat)". The step log said why: `OSError: [Errno 7] Argument list too long: 'grok'`. The steward prompt rides the harness command line as one argument (`-p "$PROMPT"`, every template), Linux caps a single argument at 131,072 bytes, and the package's findings section grew with the pending set — 123 KB on the last success, 157 KB on the first death, 1.1 MB by the time it was found — while the family map was already capped in members but not in bytes. No model was ever called.

Rulings. **The package is built to a byte budget** (`prompts.STEWARD_PROMPT_MAX_BYTES`, less the fixed identity and playbook text): the open members always ride (they are the routing targets), then the findings fill source by source in a stable order — at least one source always — then the terminal members' rows take what is left; the package ends with a line naming what it left out, so the steward never mistakes a partial family for the whole one. **The rest stays pending:** the run carries only the batches it serves (`Run.steward_batches`), the drain discards only those, and the next free slot serves the next slice — a large backlog drains in runs, not in one impossible run. **The entrypoint refuses first** (docs/07 §4, exit 17 `DEV_PROMPT_TOO_LARGE`): any prompt past the kernel's ceiling is refused with the byte counts before launch, as its own counted class, instead of dying at `execve` and surfacing as a heartbeat loss. Passing the prompt by file or stdin instead of argv removes the ceiling altogether and is the follow-up under the adaptor contract (docs/08).
