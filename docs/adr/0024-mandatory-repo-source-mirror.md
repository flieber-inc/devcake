...
6. **An empty repository bootstraps.** With no HEAD symref and no branches there is no default to inherit; a blank card's mirror takes `main` as its HEAD, the sync is green, and the resolver serves that name so the first push (a Dev's first commit, the claims writer's first notebook) creates the branch; a pin keeps its own name. A repository that has branches but advertises no HEAD symref still asks for a pin. Forges that set the default branch from the first push (the three supported ones) then agree with the mirror; a bare remote whose unborn HEAD names another branch asks for a pin at the next sync. Every remote HEAD/ref probe — the sync's own symref probe, the connection test, Discover — is bounded (20 s), a cancelled or timed-out probe kills git's whole process group (transport helpers included), and the bulk Discover answers inside the admin proxy's window with finished cards and "timed out" for the rest.
7. **Own writes invalidate the window.** `sync_max_age_seconds` covers passive staleness only. When DevCake itself changes a repository, or learns that it changed — a run finishes on its work repository (the Dev may have pushed), a mission completes because its pull request is merged (the app's own merge or one found merged; the completion chokepoint of ADR-0034 is the one place), the claims writer pushes a notebook, or a Clear prunes the claims — that card's mirror freshness is dropped (`RepoCache.invalidate`), so the next dispatch resyncs regardless of the window. The feed memo's rule — our own posts bump a generation — applied to mirrors: what the app knows it wrote is never served stale. The rule holds across a sync in flight: the invalidation records its moment, and a sync that started before it does not stamp freshness on completion (the refs it fetched may predate the write), so the next gate fetches once more rather than serve a checkout without the push. A Dev's working copy is only seeded from the mirror — its origin is the real forge and mid-run pulls go there — so the window only ever delays what a checkout starts from.
