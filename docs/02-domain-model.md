...
| `skills` | `list[str]` | Skill-store skills **installed** (Available) in the Dev container before harness launch, into the harness's registry-declared skills directory (`harness.py` `skills_dir`: claude-code → `~/.claude/skills`; grok-build/codex → `~/.agents/skills` — `08-harness-templates.md` §7a). Consult is **optional** by default (model description-match). A harness with no `skills_dir` skips them with a warning. Names validated (`^[a-z0-9][a-z0-9_-]{0,63}$`, or `<card>/<skill>` with a repo-card prefix for **external** skills served read-only from that card's ADR-0024 mirror — ADR-0016 addendum; basenames must be unique across the selection, external payload paths flatten to the basename dir), deduped preserving order. A selected-but-missing skill is skipped with a warning, never a refused run — but a skill-source CARD that cannot sync defers dispatch (the fail-closed mirror gate's needed-set union). Skills are **domain modules** (additive, not mission-step scripts) — **ADR-0016**, `app/devcake/skills/README.md`. |
...
| `context_sourcing_strict` | `bool` (default `true`) | PLAN_MEMORY: skill-source cards and memory notebooks fail-closed at dispatch when true (provisioning family, no attempt). When false, a last-good mirror is used (`stale_cache`) and a never-synced card is omitted; the run continues. Work / reference / blocker extras keep their existing rules. Amends the ADR-0016 addendum. |
...
| `budgets.claims_queue_max` | `int` ≥ 0 (default 50) | PLAN_MEMORY: max `.claims/*.json` files per notebook. `0` = unlimited. At cap the conveyor refuses the new id (does not evict). |
