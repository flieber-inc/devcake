...
Exit criteria:
- [x] Per-mission repo resolution decided (**founder decision 2026-07-14: config default per PMO instance + `` `devcake-repo:<name>` `` description-marker override**) and implemented (`domain/repo_routing.py`) — **STICKY once a run exists**: the latest run's `repo_ref` wins; a conflicting **marker** edit GATES with a human-action reason (PR-reuse invariant preserved), while a changed instance **default** does not — sticky wins silently (founder decision 2026-07-14 / audit A25; a config default edit must not park every in-flight mission). The resolved repo's adapter + credentials wire per-run into the runspec at request time (`runspec_secret_payload` by `run.repo_ref`; nothing secret at rest).
...
- [x] **Isolation (INV-2 spirit) — live-verified:** mission A's write token → own repo 200, → mission B's repo **404**; A's read token reads 200 but writes **403**. Mechanism (honest): per-mission machine user + write/read scoped token pair in a private org (Gitea tokens are user-scoped — ADR-0010, docs/14 §2 Zone B).
...
- **The delivery destination** (ADR-0017 addendum): every mission's change
  set rides a pull request; ONBOARD may declare where it lands — the
  repository (default) or the **ticket** (files attached at REVIEW approve,
  Done that says no repository changed, the pull request closed unmerged via
  the new `ForgePort.close_pr`). The destination is a description marker the
  app writes once and only a person changes; later Dev declarations are
  proposals (`DEVCAKE-NEEDS-HUMAN`); decomposition children carry it from
  birth; the merge sweep honours a late `to=ticket` on a parked pull
  request. Hardening that ships with it: an `executed` with no pull request
  is `DEV_BAD_OUTPUT`; a parked mission with no pull request is handed back
  by branch name, never wedged; description appends go through one
  chokepoint (`feed.append_note`). Retired: the deployment-global
...
- **Nested Podman store on the workspace bind** (#482, 2026-09-23): rootless
  ...
  cgroup CPU and memory budget to the tools; the Dev prompt says
  `nproc` and `/proc/meminfo` are the host; `MemorySwap` equals
  `Memory`. The probe records native overlay without flipping `rig_ok`.
  Mechanism measured on the v0.6.9 host kernel with this config applied
  to `devcake/dev-grok-build:v0.6.9-1.0.40` (native diff, zero fuse
  processes, 25 MB after an Alpine pull). **built** · **⏳ live-pending**
  the re-pin bake. Docs 07.
