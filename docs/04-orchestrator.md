...
1. Fetch all Projects and Issues in the instance's team (terminal included) via `PMOPort.list_all(team_ref)` and normalize to `Mission` DTOs. The fetched set is the **cycle snapshot** (`BoardSnapshot`): enumeration reads later in the cycle — step 3's child pre-check, step 8's in-flight scan, the dispatch-time ancestor offer — reuse it instead of paying the vendor again (ADR-0003 amendment; quotas are metered, ADR-0040). A decision that mutates a mission still pays its own live read.
2. Derive each Mission's type per the table in `02-domain-model.md` §2.
3. **Project auto-completion sweep:** for each Project carrying `DEVCAKE-TRACKING`, check its child Issues; if all are `done`/`canceled` (and it has ≥1 child), set the Project's status to `done` **then** remove `DEVCAKE-TRACKING` (status is the derive commit — same ordering as the merge sweep). **No completion comment** is posted — the status + label change is the signal. (State is derived entirely from the PMO — no local tracking.) The child read is a flat in-project filter, so second-level decomposition composes correctly: a child canceled in favor of its own sub-missions counts terminal, while the sub-missions — created inside the same Project (`adr/0012`) — hold it open until they finish. An empty child list never completes. The live `children_of` read is paid only when the cycle snapshot shows every team-local child terminal, or knows no child at all (vendors without `parent_ref`, cross-team projects); while a snapshot child is still open no vendor read is due, and completion is always decided on the live read, never on the snapshot. If `children_of` raises (e.g. a project-kind ref on a `projects_supported=False` forge-issue adapter — port F1), the project is **not** completed and the reason is recorded in `blocked_reasons` for `/health` (not log-only).
4. **Merge sweep:** for each non-terminal Mission carrying `DEVCAKE-MERGE` (forge-issue `backlog` included), check its PR via the forge adapter (`get_pr_by_branch`/`pr_state`, which return normalized `PullRequest` DTOs — attribute access, never raw forge JSON; the branch→PR lookup is memoized per parked mission for the process's life, since the number is a stable fact once the PR exists, so a cycle pays the state read alone — a number the forge no longer knows is looked up again, and a terminal answer on a memoized number is confirmed by the live lookup before the sweep writes, so a newer PR on the same branch wins exactly as it did without the memo) — merged → status `done` **then** remove the label (status is the derive commit; a failed status leaves the label so the next cycle still selects); closed unmerged → cancel **then** remove the label; either way with a comment (`03-mission-lifecycle.md` §4.1). While the PR is still open and a deferred-merge retry window is active (that mission's repo has `auto_merge` ON, latest merge-state feed marker is `` `devcake:merge-retry` ``), the sweep also drives the retry: `mergeable()` each cycle — ready → merge and complete; conflict → conflict-rework routing; computing/CI → wait; window elapsed → admitted probes continue (and the merge itself when the forge says ready); the second probe in a row that does not merge ends in a terminal hand-off (posted once), so one transient forge error is never terminal. When the latest merge-state marker is `` `devcake:merge-settle` `` instead, the sweep first waits `merge_settle_minutes` and re-runs the freshness recheck (which may re-open REVIEW) before opening the retry/merge — full driver table in `03-mission-lifecycle.md` §4.1. Done is only ever declared after the merge is real — or, for a mission whose record says `to=ticket`, after its files are on the ticket (ADR-0017 addendum: the sweep delivers, completes and closes the pull request unmerged when a person writes that line onto a parked mission). A parked mission whose branch carries **no pull request** is a visible `blocked_reasons` wait while its deferred window is open, and a single `` `devcake:merge-handoff` `` hand-back (banner kept, nothing canceled) once the window has elapsed or when none was opened (`03-mission-lifecycle.md` §4.1). The merge-state markers — like the discovery sweep's board arithmetic — are re-read from the feed only when something changed (`FeedScanMemo`: DevCake's own feed writes invalidate the memo through the feed chokepoints — a post through `_feed`, an edit of its own entry through `_edit` (ADR-0042), a changed `updated_at` invalidates it, and on a vendor whose `updated_at` does not move for comments (`PMOCapabilities.updated_at_tracks_comments` false) a five-minute safety rescan catches a human's comment); the memo decides *when* the feed is recomputed, never *what* it says; a truncated full-history scan (the discovery arithmetic) is never memoized, and the sweep confirms on a live scan before it writes. On a vendor that can list a team's feed changes since a moment (`PMOCapabilities.feed_delta`; `feed_changes_since`), one changes read per cycle stands in for the per-mission re-reads: the read carries ids and times, never text, and a memoized scan whose mission changed is kept when no entry of that feed changed after the scan's newest entry (a label, status, or relation edit costs no feed read) and dropped when one did (a comment, an edit, a removal the vendor lists — the next scan pays the read, once: the reported change time becomes that mission's floor, so the same row re-delivered on later reads does not pop the rescan). The scan's age is not refreshed by a witness: a vendor that keeps the safety rescan keeps it, and the rescan catches what the witness cannot list (a removed comment on a vendor that does not report removals). A page-capped changes read drops every memoized scan for that cycle; one the request budget refuses changes nothing and the per-mission rule applies; a permanent failure switches the witness off until the next settings Save or a restart. The read is skipped while nothing is memoized (the watermark then follows the board's newest change time), and the watermark is always the vendor's own clock, never this process's.
...
8. **Scheduled-task fires (`CronService.maybe_fire`, ADR-0035):** for every enabled `crons` row whose elapsed-interval window (persisted `last_fire_at` in `state/cron_outcomes.json`) is due and that is not degraded (last 3 automatic fires failed — ledger-derived, restart-safe; Run-now always works and a success re-arms), create ONE labeled ticket (`DEVCAKE` + stage label, `devcake:cron:v1 job=<id>` marker, single-flight per board, intake-pause honored — a scheduled fire held by the pause or by an open ticket records a skip; the operator's Run now is told that reason (409) instead of an empty result, and never bypasses the pause). The reserved `memory-curator` row instead fans out one EXECUTE ticket per Curator board, skipping notebooks whose `.claims/` listing is empty. One outcome (`created`/`skipped`/`failed`) is recorded per fire window. Exceptions never kill the cycle.

Per-mission repository resolution is **sticky once any post-triage run exists** (PLAN/EXECUTE/REVIEW — a plan is bound to one tree even before code is written): a conflicting marker edit gates with a visible reason. ONBOARD is read-only triage with every work repository mounted, so an ONBOARD run never latches the repository: a marker added after triage, or a changed instance default, takes effect at the first post-triage step (`domain/repo_routing.py`).
...
- have an **open blocker** (`adr/0007`): any Mission in `blocked_by` whose normalized status is not `done`/`canceled`. The check resolves blockers against the poll snapshot (terminal Missions included, so done blockers resolve); a blocker outside the snapshot resolves once per cycle (memoized) through the deployment-wide `BlockerLocator` — owner map, then the local adapter, then same-system PEER instances when the local adapter declares `PMOCapabilities.global_ids` (ADR-0009 amendment) — so a native edge to a peer instance's mission gates identically to a local one; a mission's whole off-snapshot set goes through one locator walk (`resolve_many`), and an adapter that declares `PMOCapabilities.batch_get` answers it in one read per hundred ids, so a mission gated on hundreds of finished siblings costs a handful of requests, not one per edge; a blocker no path can read counts as open — fail-safe, self-healing next cycle. Cycle detection stays on the local instance graph (a cross-instance edge cannot close a local cycle). A blocker carrying `DEVCAKE-FAILED`/`DEVCAKE-SKIP` is still open: the prerequisite will not complete autonomously, so dependents stay parked and the reason string names the guard label (surfaced in `/api/v1/missions` — this makes blocked-on-a-dead-blocker deadlocks visible). **Blocked-by is re-verified live at dispatch** (§3.1) — the pre-launch gate and the blocker-work pass share one memo, so the set is read once per dispatch; a live reopen surfaces the same `blocked by …` reason on the missions row. Because it honors *any* blocked-by relation, humans steer ordering by adding/removing relations in the PMO UI — no DevCake-specific knowledge needed. Treating a `canceled` blocker as satisfied stays sound under decomposition because inherited edges are **fail-closed**: the finalizer replicates a decomposed original's edges onto its children strictly before canceling it, and any edge failure keeps the original open and gating (`03-mission-lifecycle.md` §1.3, `adr/0012`) — dependents hand over from the original to the still-open children with no released window.
...
- **Timeout:** if `now - created_at > timeout_seconds` (default from `dev_timeout_minutes` at dispatch, 120 min), kill the run via Dagu's stop endpoint — `POST /api/v1/dag-runs/dev-run/{run_id}/stop` (SIGTERM → SIGKILL after `max_clean_up_time_sec` → container force-removed, run `aborted`) — and mark the Run `timed_out`. Counts as a failed attempt (`15-errors-and-retries.md`, `DEV_TIMEOUT`). Ages are measured from **`created_at`**, not `started_at`. Dagu-side timeouts are intentionally not relied on — the app owns the kill (single owner; `13-deployment.md` §4 sets a belt-and-suspenders `timeout_sec` well above the app's). The app needs no `docker.sock` for this. If `unresolved_run_ids` still lists the run **and** Dagu is already dead (`dagu_run_not_alive`), promote to `finalizing` instead of killing — a first delivery must not race the terminal guard (CAKE-73). Membership in `unresolved_run_ids` alone is not enough: that probe covers any ingress kind (heartbeat every 30 s, logs, chunks, artifacts), so a still-alive timed-out Dev is still killed; handle-side empty `finalized_steps` reopen covers a first delivery that races after the kill.
...
3. **Reconcile Runs** (`domain/reconcile.py`): for each local Run in a non-terminal state, query the Dagu run-status API for `run_id`:
   - Dagu says finished + artifacts message present in Redis (pending-entries reclaim, `09-messaging.md` §5) → promote to `finalizing` and resume finalization from `finalized_steps` (consult `MessagingPort.unresolved_run_ids`; do **not** orphan first — orphaning then reclaim would hit the terminal-state guard and drop a first delivery, CAKE-73).
   - Dagu says running → adopt it: mark `running`, resume watchdog coverage.
   - Dagu status/label is in the exact dead set `failed` / `aborted` / `error` / `cancelled` (British spelling on the label arm), or the API returns no such run, **and** no unresolved ingress entry for the run → mark `orphaned` (`DEV_ORPHANED`); counts as a failed attempt; the Mission (whose label never advanced — INV-3) reschedules naturally. American `canceled` is **not** in this exact set (watchdog §5 uses a `cancel` substring and would treat it as dead mid-flight).
   - A `dispatched` Run with no Dagu run → the §3.1 crash window; mark `orphaned` (never re-trigger blindly — a duplicate `dagRunId` trigger returns 409 `already_exists`) unless artifacts are still unresolved (same promote-to-`finalizing` path).
