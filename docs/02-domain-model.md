...
| `harness_template` | a `HARNESSES` key (`claude-code`, `grok-build`, `codex`, `pi`, `opencode`, `qwen-code`) | **Authoritative** (2026-07-12 rework; H2): the Docker image, credential requirements, and OAuth flow all derive from it via the harness registry (`app/devcake/harness.py`, `08-harness-templates.md` §2/§4). Changing it in the admin panel changes what actually runs. Config validates against the registry — no parallel Literal. |
...
| `cli_version` | `str` | Coding-harness **binary** pin (not `DEVCAKE_TAG`, not `model`). Empty = house Dockerfile ARG. Stored value is a concrete semver; the token `latest` is a resolve-once gesture and is 422 if persisted. Experimental templates refuse a non-empty pin. Save publishes the keep-set; the host baker compiles and probes. Dispatch refuses until a matching ok receipt exists. |
