...
export default function TrafficSection() {
  ...
  const [stewardMsg, setStewardMsg] = useState("");

  const cfg = dr.draft.cfg;
  const setField = dr.setField;

  const runSteward = async () => {
    setStewardMsg("Starting…");
    try {
      const r = await send("POST", "/steward/run");
      setStewardMsg(`✓ dispatched ${r.run_id} — watch it on the Runs page`);
    } catch (e) {
      setStewardMsg(`✗ ${String(e.message || e)}`);
    }
  };

  const rm = cfg.steward || { enabled: false, interval_minutes: 60, dev_type: null };
  const serverRm = dr.server.cfg.steward || {};
  const stewardDirty = dr.diff.some((x) => x.path.startsWith("cfg.steward"));

  return (
      <Section id="traffic" title="Traffic control"
        description="Mission breakdown depth and the Relations Steward. (Mission intake is the master switch in the sidebar — it applies immediately.)">
        ...
        <div className="space-y-3 rounded-card border border-neutral-200 p-4 dark:border-neutral-800">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button kind="ghost" icon={Play} onClick={runSteward}
                disabled={!serverRm.dev_type || stewardDirty}
                title={stewardDirty
              ...
              <Toggle on={rm.enabled} label="Periodic service"
                onClick={() => {
                  if (!rm.enabled && !rm.dev_type) {
                    setStewardMsg("✗ pick a Dev Type first");
                    return;
                  }
          ...
          {healthInfo?.steward_degraded && (
            <p className="text-sm text-amber-600 dark:text-amber-400">
              ⚠ Periodic service backing off — the last 3 steward runs failed
              ({healthInfo.steward_degraded}). Run now still works; a successful run resumes
              the schedule.
            </p>
          )}
          {stewardMsg && (
            <p className={`text-sm ${stewardMsg.startsWith("✗") ? "text-red-600" : "text-green-700 dark:text-green-400"}`}>
              {stewardMsg}
            </p>
          )}
        </div>
