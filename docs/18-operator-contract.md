...
| **Rebuild in lockstep on upgrade** | Every time `app/`, `admin/`, or `images/` change | `docker compose stop dagu` then `devcake up --bake` so the live DAG bind cannot see a new `dev-run.yaml` without `DEVCAKE_WS_HOST` ([`13`](13-deployment.md) §8, `AGENTS.md`). Default `--bake` is control plane + hello; the host baker compiles configured harness pins. |
...
2. **Model / harness credentials** — upload via the Dev Type card under
   Fleet (OAuth wizard, credential upload, or `scripts/grok_login.sh <dev-type>`
   — pass the operator-chosen name; `DEVCAKE_DEV_TYPE` or a positional arg;
   app container via `DEVCAKE_APP_CONTAINER` / compose `ps`).
