...
- Component health dots from the 10 s health poll: app / pmo / redis / dagu / **gitea** (`health.internal_forge`, grey when Gitea is unset) / logs (OpenObserve) — not a generic forge dot — plus a theme toggle. Note (ADR-0030): on bundled stacks the auto-provisioned `board` instance joins the `pmo` aggregate, so the dot goes green on a fresh boot and **red when the bundled Gitea is sick** — Linear-only operators included; honest red is doctrine.
...
| `GET /api/v1/harnesses` | The harness registry: derived image, credential requirements, OAuth availability per `harness_template` — the Dev Type editor renders (and previews unsaved harness switches) from this. The Dev Types section is a roster table whose rows open an editor modal (`admin/spa/DESIGN.md` §2, re-decided from the tile grid 2026-08-02); see `docs/img/devtypes-roster/` for UI evidence |
...
| `GET /api/v1/assignments` · `PUT /api/v1/assignments` | The **global** Mission-Type → Dev-Type map. Validation: ONE shared rule (`config.validate_assignment_map`, 2026-08-12 audit SEC-3) — all four types assigned, no unknown mission-type keys, each row naming an existing Dev Type. Per-instance override rows (ADR-0019) ride `pmos[*].assignments` in PUT `/config` — shape validated at the model, dev-type existence checked unconditionally by the shared cross-store semantics (boot, PUT /config, and bundle apply run the same check; the SPA saves Dev Types before the config PUT for this reason) |
...
| `POST /api/v1/steward/run` | Manually dispatch a Relations Steward run (`03-mission-lifecycle.md` §4b). Works regardless of the `enabled` toggle (which governs only the interval service); 422 without a valid `dev_type`, 409 while a steward run is active |
...
| `POST /api/v1/profiles/{name}/apply` | THE world-swap: replaces the sections the profile contains through the config choke points (a configs-only profile keeps live secrets). **409 while runs are active**; shares the poll-cycle lock like a config PUT (an in-flight cycle finishes first); rollback-by-reapply on reload failure; appends an audit event |
...
| `POST /api/v1/settings/import/preview` | Stateless: parse (`yaml.safe_load`, 20 MB cap) + decrypt + diff vs current. `{needs_passphrase}` for encrypted bundles without one; wrong passphrase = one indistinguishable 422. Secrets deltas are names only — incl. `credential_files: {added, replaced, removed}` as `dev/file` entries (SEC-2 rider: a present `credential_files` section applies replace-the-world; absent = untouched), with a rotation warning when a host file is newer than the snapshot |
...
| `POST /api/v1/missions` | ADR-0030: transcribe an operator-originated mission onto a PMO board. Body `{instance (required), title, description?, priority?, adopt? (opt_in only), attachments?: [{name, content_b64}]}`; title/description **redacted before the port call**; labels = `{DEVCAKE}` when opt_in+adopt, never `DEVCAKE-CREATED`; per-file cap = the adapter's `attachment_max_bytes`, ≤10 files. Write-through via `PMOPort.create_mission` — **no local record** (INV-1). Uploads run after create, never aborting the loop; ≥1 success posts one sentinel-free linking comment (Linear attachments are invisible unless referenced from a post). **Partial failure = 200 with `attachment_failures`** — the mission EXISTS; no rollback (the port has no delete). 422 validation · 404 unknown instance · 409 unconfigured · 502 PMO |
| `POST /api/v1/poll/run` | Force a poll cycle now (the Missions board's "Poll now" action). Mirrors the steward "Run now" shape (§1.6): 409 while a cycle is in flight (periodic or another manual trigger); 200 with `{ok, cycle, started_at, duration_ms}` on completion. Missions live in the PMO — this closes the ~30s feedback loop after a board edit (or a composer create) without waiting for the next automatic tick |
...
## 2a. Missions page (`#/missions`)

**Pipeline strip + grouped mission list** over the current poll snapshot (`GET /api/v1/missions`) — a 2026-08-02 re-decision of the original kanban (`admin/spa/DESIGN.md` §2: column geometry assumed even stage occupancy; the steady state is extreme skew). The **strip** is a sticky row of seven stage chips in pipeline order (Backlog → Plan → Execute → Review → Merge wait → Needs human → Done) with live counts — populated chips are jump buttons to their section, Needs human goes amber whenever non-zero, and empty stages exist **only** here. Below, one **section per non-empty stage** — Needs human pinned first, Done last — each mission a single dense row: stage glyph, mono key, full-width truncating title, needs-human badge, priority dot (tooltip; chips would repeat "medium" fleet-wide), repo, age, ↗ PMO link, and the **MoreMenu**: **Park** / **Retry** (when `DEVCAKE-FAILED`) / **Resume** (when `DEVCAKE-NEEDS-HUMAN`) / **Unpark** (when `DEVCAKE-SKIP`) via `POST /missions/{id}/actions`. A reason every row of a section shares (e.g. "terminal — ignored") is hoisted into the section header; rows spell out only deviations. **Done previews its 10 newest with "Show all N"** (`bucketize` itself caps Done at the 30 newest). **Per-stage visibility (2026-08 reviewer round):** every section header carries a **Hide** affordance; a hidden stage disappears from the LIST only — its strip pill stays, dimmed (dashed border), still carrying the live count, so a stage with work in it can never become invisible-invisible, and clicking the dimmed pill shows the section again. The choice persists per browser in localStorage (`devcake-missions-hidden`, the `devcake-sidebar` precedent) — a view preference, never config. **PMO provenance + filter (multi-PMO only):** when ≥2 PMO instances are configured (ADR-0009's N>1 convention — the gate and option list ride the `teams` map the same `/missions` response carries), every mono key gains a dimmed `instance·` prefix (row + drawer; a chip would repeat fleet-wide with no scan value, the priority-dot argument) and the strip gains a **Filter by PMO instance** select. The filter narrows the list AND the strip counts together (pills are jump buttons — a count that disagrees with its section reads as a bug) with an honest `n of m missions` indicator; an instance with zero missions gets dedicated filtered-empty copy (never the bootstrap empty state) plus **Show all PMOs**. Persisted in localStorage (`devcake-missions-pmo`) as a view preference; below the two-instance threshold — or naming a deleted instance — a leftover filter is **forcibly inert**, so it can never hide missions invisibly (the §2b threshold rule). Row click opens the drawer: **Send guidance** (`POST /missions/{id}/comment`) and per-run **Stop** (`POST /runs/{id}/stop`). **Dependency chain panel** (first section of the drawer body, issues with ≥1 edge only): a read-only three-band chain — *Blocked by (n)*, the current mission highlighted, *Blocks (n)* — derived client-side from the snapshot's `blocked_by` keys (dependents by inversion; transitive ring behind a *Full chain (k more)* disclosure; div/border construction, no SVG). Resolved nodes are buttons that navigate the drawer along the chain (the drawer remounts so a half-typed guidance draft never posts to the wrong mission). Honesty rules: raw vendor ids (blockers aged out of every snapshot) render as inert *no longer in any snapshot* nodes; `done`/`canceled` blockers dim as *no longer blocking*; guard-labelled blockers go amber naming the label (*failed (DEVCAKE-FAILED) — blocks until a human acts*); cycle members (from `/health.dependency_cycles`, both bare and `instance:key` forms) get the *will never start until a relation is deleted in your PMO* line; project drawers state *Projects carry no dependency relations — dependencies live on their issues*. Strictly read-only — DevCake never adds or removes relations (ADR-0007/0012); the escape is *Manage in PMO ↗*. **Header actions (ADR-0030):** the single primary is **New mission** (the composer — instance select over configured PMOs, title/description, priority hidden where the adapter's registry row says `supports_priority: false`, an adopt toggle under `opt_in` only, file attachments, and a ConfirmDialog with `confirmKind="primary"` whose copy is assembled from the actual request); **Poll now** rides beside it as a ghost (`POST /api/v1/poll/run` — 409 while a cycle is in flight; a live action never buries in a menu, §3). Post-create: an honest flash names what exists where (failed attachments disclosed by name) and a silent follow-up poll fires — and when the created mission is **unadopted** the flash says so instead of promising it "appears on the board", because `bucketize` drops unadopted rows entirely. **Adoption-gate counter (2026-08-05 founder report):** under `opt_in`, missions with no `DEVCAKE` label render in NO section — a freshly created unadopted mission simply vanished — so the page states `n missions are not adopted (no DEVCAKE label) and are not listed below` above the sections (`unadoptedHiddenCount` in `lib/board.js`, defined against `columnOf` so it can never drift from the real drop rule; hermetically covered by `tests/board_helpers.mjs`). The list works at any viewport width — no horizontal scroll, no sidebar force-collapse. See `docs/img/missions-board/` for UI evidence.
...
**Collapsed summary rows (2026-08 reviewer round):** each repo card collapses to a one-line summary — mono name · forge · URL host · merge posture ("auto-merge"/"merge hand-off") · last connection-test ✓/✗ — that expands in place to the full card. Fleets of **≤3 start expanded**; larger fleets start collapsed; a newly added card opens expanded (its form needs filling); an expanded card's ▾ collapses it back. Expansion is ephemeral per-page view state (index-keyed, so renaming a repo mid-edit never collapses the card being typed in). The same treatment applies to the PMO page's instance cards (`PmoSection`, `data-testid="pmo-summary-row"`), which also gains the name filter below.

The name-filter input (debounced, ✕ clear) appears past **5 repo cards** (2026-08 reviewer round — it used to wait for the render cap; below the threshold it stays hidden, so a leftover filter can never hide cards invisibly). Past **30 cards** the page additionally engages its bulk mode (2026-08-02): a render cap of 30 + a "Showing 30 of N — refine the filter" note, and the cards' token ✓/✗ states come from **one chunked `secrets-check` batch** for the rendered set instead of per-field fetches (~4 requests per card previously — ~1,400 at the 350-repo stress test). The PMO page's filter mirrors the 5-instance threshold (filter only — PMO fleets don't reach cap scale).

**The managed board card (ADR-0030):** the auto-provisioned `board` instance renders as a normal-but-marked card — a "Bundled board" badge (with a help popover owning the truth: identity and API key are app-managed and self-healing; a config save that omits the row puts it back), identity fields (system, team key, API base) disabled, the API-key SecretField replaced by an honest "app-minted, nothing to paste here" note, and **Remove hidden while the bundled provisioner exists** (`health.internal_forge` non-null). Intake toggle, repositories, reference repos, and staffing overrides stay fully operator-editable — empty repos = per-mission internal-forge repos = the fully standalone path.
...
- **Relations Steward card** — four controls: (a) **Run now** button → `POST /api/v1/steward/run`, showing the dispatched run id (or the 409/422 error) inline; (b) **interval in minutes**; (c) **Dev Type combobox** (defaults to the seeded `steward`; required before enabling or running); (d) **Periodic service ON/OFF toggle** (default OFF — manual-only out of the box). Controls disable while a save is in flight (no stale-state races). The card shows the **degraded** state from `/health` (`steward_degraded`). Deleting the steward's Dev Type is refused with 409.
...
**Per-run Stop (2026-08 reviewer round):** each **dispatched/running** row carries a trailing **Stop** button — the MissionDrawer's Stop lifted onto the table (same `POST /api/v1/runs/{run_id}/stop`, same eligibility: finalizing rows show no button because the Dev has already exited and the backend 409s a stop there by design, same honest confirm copy — the run is recorded as failed and counts toward the retry limit; park the mission first if a retry is unwanted). Terminal rows show nothing.
...
- `RunBootstrap.dispatch_lock` — **every** dispatch flavor (poll, hello, OAuth, steward “run now”) creates its Redis ACL user + starts the container inside `launch()` under this lock (PR #31). Holding only the poll lock is **not** enough.
