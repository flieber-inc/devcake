...
- The codex and grok predicate arms are designed from `docs/08` and the parsing code; only `claude-code` has real captures. They stay provisional until a capture session supplies streams (`turn.failed` is a named gap).
...
- `docs/07-dev-runtime.md` §4 (exit codes), `docs/15-errors-and-retries.md` §1/§2/§4a, `docs/09-messaging.md` §3 (`run.artifacts` now documents `exit_code`/`error_class`/`error_detail`), `docs/03-mission-lifecycle.md` §3 (INV-6 scopes code changes, not outputs), `docs/11-admin-panel.md`, `docs/12-observability.md`.
- Implementation: `images/common/dev_entrypoint.py` (`harness_fault`, `workspace_forensics`, `find_result_json`, `bad_output_reason`), `app/devcake/domain/backend_health.py`, `finalize.dev_failure_error`, `dispatch.counts_toward_attempts`, `runs.KILL_CLASSES`, `schedule.schedule`, `poll.run_cycle`.
- Evidence: `app/tests/fixtures/harness_streams/` — four live Claude Code 2.1.219 captures with a README recording why token counts are not a fault signal.
