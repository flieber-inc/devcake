...
**Loop guardrail (warnings only — never auto-park):** the app counts prior `human_needed` runs for the same (mission, stage) from the run store; from the 2nd hand-off on, the baton-pass comment carries an escalating header — "Hand-off #N on this step … add `DEVCAKE-SKIP` to stop DevCake on it." DevCake never parks on its own; the human always decides.
