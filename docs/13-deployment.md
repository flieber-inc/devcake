...
- **Redis posture (2026-08):** the service runs `--maxmemory 1gb --maxmemory-policy noeviction` (a flooding Dev gets write errors instead of OOMing the control plane; the app XDELs ingress entries after every handled batch, so legit load never nears the cap) and `--aclfile /data/users.acl` (per-run Dev users survive a redis-only restart; the app `ACL SAVE`s on every user create/delete). **Rotating `REDIS_PASSWORD`:** the aclfile carries the `default` user after the first save, so a `.env` change alone is not enough — while the stack is stopped either delete `/data/users.acl` (in-flight-run users go with it) or run `ACL SETUSER default >newpass` + `ACL SAVE` against the live instance first.
