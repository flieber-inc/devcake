...
- **Harness fault classification + model-backend brake** (2026-07-24, ADR-0018): the Dev entrypoint stopped inferring failure from the process exit status alone — it now inspects each harness's own terminal event, so a backend answering HTTP 200 with an empty completion is reported as `DEV_HARNESS_FAULT` (exit 15) instead of being laundered into `DEV_BAD_OUTPUT` on every container at once. Adds exit 16 `DEV_TURN_BUDGET`, structured `error_class`/`attempt_counted` on every terminal path, bounded workspace forensics on exits 10/11/15/16, misplaced-`result.json` diagnosis (recovery behind `recover_misplaced_result`), a store-derived per-Dev-Type throttle with per-step excusal caps, and fixes to the chunk-buffer ratchet, the injectable attempt-counting match and the EXECUTE binding-rule contradiction. **built** · 811 unit tests green · ruff clean · bake app/admin/images.
  > **Correction (2026-07-25):** as merged, the predicate was only correct for the harness it was developed against — `empty_completion` was structurally unreachable for codex and inverted for grok, exit 16 was reachable for `claude-code` alone, and a codex/grok 401 regressed onto exit 15. A 27-capture campaign across all three CLIs measured it and a fix round closed every row; see the ADR's 2026-07-25 amendment and `app/tests/fixtures/harness_streams/README.md`.
