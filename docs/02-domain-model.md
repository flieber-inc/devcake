...
| `cli_version` | `str` | Coding-harness **binary** pin (not `DEVCAKE_TAG`, not `model`). Empty = house Dockerfile ARG. Stored value is a concrete semver; the token `latest` is a resolve-once gesture and is 422 if persisted. Experimental templates refuse a non-empty pin. Save does not bake — dispatch refuses until a matching ok receipt exists. |
