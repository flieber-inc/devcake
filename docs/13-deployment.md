...
> `devcake status` also prints the PMO request budgets the app measures (about N requests/hour per credential against its limit, rejections by the tracker in the last hour, the alarm text when one is reached) by reading `/health` through the loopback admin proxy with the checkout's admin credentials; when the stack is down the line says so.
