...
- The app deletes the ACL user on every terminal path — finalization (idempotent step), watchdog/reconcile/operator/clear kills, and boot quarantine of a corrupt record — and clear-runs sweeps `ACL USERS` wholesale. There is **no** periodic `ACL LIST` orphan sweep at startup (AUD-008 corrected an earlier overclaim here): a crash between `create_run_user` and the durable save (a narrow window) can orphan one ACL user until the next clear-runs. Adding a boot sweep of `dev-*` users with no live Run is tracked debt, not shipped.
- **Durability (2026-08 evaluation):** every per-run `SETUSER`/`DELUSER` (`create_run_user` / `delete_run_user`) is followed by `ACL SAVE` into the compose-configured `aclfile` (`/data/users.acl` on `redis_data`), so per-run users survive a **redis-only** restart — previously they lived in redis memory alone, and an OOM or `compose restart redis` stranded every in-flight run (containers alive, auth gone, killed at the heartbeat grace). Clear-runs bulk-revokes leftover `dev-*` users through the same adapter (`revoke_leftover_run_users`) and **also** `ACL SAVE`s once after the bulk `DELUSER` set — without that, a redis restart would resurrect wiped users from the aclfile (compose boot preserves non-default lines). The save is best-effort: without an aclfile (bare dev redis) it logs a warning and the old exposure stands. **Auth ownership moved with it (measured 2026-08-04):** with `--aclfile` configured redis IGNORES `--requirepass`, so the **default** user's password lives in the aclfile too — the compose entrypoint regenerates its line from `.env` on every boot (preserving `dev-*` lines), which makes `REDIS_PASSWORD` rotation a plain restart. Restart drill verified: a `dev-*` user authenticates after `docker restart redis`.
...
|---|---|---|---|
| `runspec.get` | Dev→app | `{phase?}` | Request/reply: each of a run's two containers fetches its spec independently (stateless — the reply is rebuilt from current config per request). App responds with `runspec.result` `{env: {…}, credential_files: [{path_hint, content, mode}], mcp_setup_commands: […], skills: [{name, files: [{path, content_b64}]}], skills_dir, extra_repos: […] (per entry `{name, url, clone_user}` + either `mirror_path` — ADR-0024 mirrored, NO token in transit — or `token` for direct clones; whether an entry rides the mirror is decided at DISPATCH — the gate's `needed_for` set snapshotted on `run.mirror_repos`, exactly like the primary's `DEVCAKE_MIRROR_PATH` — so a repo added to the instance mid-flight can never receive a mirror path the gate never proved; tokens are still resolved from current config at request time), memory_repos: [{name, url, clone_user, mirror_path? or token, stale_cache?, strict?}] (PLAN_MEMORY — consumer notebooks cloned to `/workspace/memory/<card>/`; dest is the card name; snapshot on `run.memory_mounts` — `{card, binding, commit, stale_cache, strict, path}` — so a notebook added mid-flight does not appear and a mid-flight `context_sourcing_strict` flip cannot change a live run's contract; `strict` on an entry makes its clone failure FATAL for the provision step — exit-13 forge family, same as the primary clone — while non-strict failures stay a printed note; Curator inherit extras stay in `extra_repos`), activity_repo: {url, clone_user, token} (null when absent), prompt}` (`activity_repo` — ADR-0014 D4: the shared-RO clone spec for the mission's activity repo; absent/null ⇒ the entrypoint materializes the folder from `activity.get` instead) — the stage-2 environment, credential material, MCP plugin setup commands, and skill-store files for this run, scoped to its Dev Type (`07-dev-runtime.md` §3, §5 steps 4b/5). **Phase scoping (ADR-0025):** the payload carries the caller's `phase`. `provision` ⇒ a REDUCED spec — the run's non-secret env, the extras entries (mirrored ones already tokenless), the activity-repo credential, and `DEVCAKE_FORGE_TOKEN` only for a direct-clone work repo (`mirror_path` empty); `credential_files`, harness/model keys and Dev-Type secret env are `[]`/absent, because the provision container runs no agent. `harness` (and, defensively, a no-`phase` request) ⇒ the full spec above. **Secrets built on request:** the secret half is derived from current config the moment an authenticated *active* run asks (nothing secret at rest between dispatch and `runspec.get`, no TTL to expire under a slow container start or Redis restart). **Secrets in transit:** the app `XDEL`s the reply entry as soon as the Dev acknowledges (`runspec.ack` below), and the reply stream carries a short TTL (§5); this channel exists because Dagu params are UI-visible (`14-security.md` §4). |
...
| `runspec.error` | app→Dev | `{error}` | App cannot build a runspec for this run; entrypoint prints the error and exits 20. Includes host-side Grok OAuth refresh failure (reconnect via admin OAuth) — fail closed so the Dev never boots on a known-bad session. |
| `run.started` | Dev→app | `{container_hostname, harness_version?}` | Sent by the **provision** container after its `runspec.get` (ADR-0025 — the sole sender; the harness container relies on its heartbeat). `harness_version` is the first line of `<cli> --version` (capped 120 chars; omitted when the probe fails). App marks Run `running` and stamps `started_at` (and `harness_version` when present) ONLY on the `dispatched→running` transition — every replay (redelivery, XADD retry, Dagu-UI re-run) is dropped, which also prevents a stray `run.started` from reverting a `finalizing` run or clobbering `started_at`. |
...
- **Safe redelivery.** A redelivered `run.artifacts` re-enters finalization; already-applied side effects are skipped via `run.finalized_steps` (`04-orchestrator.md` §4). Terminal-state guards refuse to re-open finished/failed/timed_out/orphaned runs **that already have finalize work recorded** (`finalized_steps` non-empty). A first delivery that lands after a premature orphan/kill (empty `finalized_steps`) re-opens finalize so INV-5 is not dropped (CAKE-73). There is no blanket global `(run_id, kind)` dedupe table.
