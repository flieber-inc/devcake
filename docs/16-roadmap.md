...
- **Per-board plan approval** (2026-09-02, founder ask): a PMO-card toggle
  ...
  `DEVCAKE-NEEDS-HUMAN`, so a person reads the plan before any code is
  written; approving = removing the label (or Resume), changing = guidance
  comment + relabel back to PLAN. Reuses the hand-off label and recovery
