...
## Addendum — `devcake up` never bakes a Dev image (2026-09-08)

The first `--release` implied `--bake all`, the bake file's full matrix: with `images/` changed, a host spent ten to twenty minutes rebuilding six house-pin harness images no Dev Type dispatches on, while the host baker was building the two pinned images the fleet actually runs. Ruling: **`devcake up` bakes the control plane only** — app, admin, hello, and `app-test` for CI — and **refuses** any other `--bake` target with the reason and the remedy. Dev images have one owner, the host baker: the keep-set the app publishes is the order, receipts are the registrar, and a tag move is a bake order the baker honours itself. A CLI bake of a harness would race that build for the same pin and buy nothing. `--release` therefore implies the control-plane bake; `docker buildx bake all` by hand stays the development way to compile the matrix.
