...
Which Dev image a run uses is `resolve_image(dev_type)` (`08-harness-templates.md` §2); `docker_image` is no longer stored config. The **host baker** (`scripts/dev_factory`, started by `./up.sh`, pidfile `.factory/watch.pid`) is the bake verb: it reads the app-published keep-set, validates it, compiles, probes, writes receipts and `/data/harness_bake_status.json`. Each 5s tick (the same loop, not a second supervisor) probes the app's `/health/live` the way `up.sh` does — if the app is down the baker **exits**. The app can stay up if the baker dies; `/health` then reports `baker_alive: false` and the admin paints a red critical warning (same place as a circuit breaker) telling you to `./up.sh`. Heartbeat older than 30s is dead — including a crash that left `state: baking`. The app never talks to Docker. Dagu never bakes. A Claude-only keep-set does not grow Qwen.
