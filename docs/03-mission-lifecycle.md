...
**The release is visible to the Dev (a resume is a human's answer).** Only a person removes `DEVCAKE-NEEDS-HUMAN`, but a label removal is not a feed entry, so the next run could not see that it had been released and — when its brief expected an approval — asked again. Now, when a mission's most recent finished run was a genuine hand-off, the next run's `MISSION.md` opens with a one-line pointer and its activity mirror with a `▶ RESUMED BY A HUMAN` banner: which step handed off, when, the exact ask, and how many 🧑 HUMAN comments that run never saw — its feed watermark is the boundary, so an answer posted while it was still running counts (`07-dev-runtime.md` §2). Only a hold qualifies: a run halted by an external stage change, or parked for an illegal outcome, never triggers it, and ancestor mirrors never carry it. The playbooks state the convention next to the hand-off instruction: a human comment since the hand-off is the answer; without one, the release itself is the answer — an approval or decision asked for is granted, something asked to be provided should now be there and is verified — and the Dev hands off again only if the obstacle demonstrably persists. A brief therefore never needs to invent its own approval protocol.
