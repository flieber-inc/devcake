...
| **Harness Template** | A registry-backed runtime adapter (`claude-code`, `grok-build`, `codex`, `pi`). A template selects the CLI image, credential/OAuth contract, invocation and artifact parsing, and skills directory — not a fixed model. `DevType.model` selects the model; empty uses the registry or CLI default. Specified in `08-harness-templates.md`. |
