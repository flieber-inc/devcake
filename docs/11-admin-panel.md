...
- **Secret env vars**: free-text area, one NAME per line (UPPER_SNAKE_CASE), plus one paste field per listed name (values via `harness-secrets/{VAR}`, ✓/✗ from `dev_type_status.secret_env_present`). Delivered into the run's env so MCP commands can reference `$VAR` without a value ever entering config (ADR-0011). Presence is informational only — it never gates dispatch or `credentials_ready`.
