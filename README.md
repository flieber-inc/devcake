...
> **Status:** implementation in progress per [`docs/16-roadmap.md`](docs/16-roadmap.md) — **M6 functionally complete** — the real admin panel is live: full Config CRUD (Vite/React SPA), guided OAuth wizards for harness logins, credential management, connection tests, and the GitLab adapter (live GitLab verification pending a sandbox). Next: M7 (hardening + unattended acceptance).
