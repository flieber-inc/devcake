...
- **Visibility:** `/health.pmo_budget` (one row per bucket: limit, remaining, reset, waits, refusals, rejections seen, demand per hour per instance, quota spent by another consumer of the credential) and `/health.pmo_budget_warnings` (measured demand above a share of the limit → the poll interval that would fit; a shared-credential note), rendered as a dismissable admin warning (`11-admin-panel.md`), and `/health.pmo_rate_limited` (the tracker itself rejected requests in the last hour, or is refusing now — count, instances, when the pause lifts, the interval that fits), rendered as a critical, non-dismissable, self-clearing alert; each PMO card shows a one-line readout of its share, and `devcake status` prints the same rows on the host. Waits of a second or more emit `pmo.budget.wait` (`12-observability.md`). `DEVCAKE_PMO_BUDGET_OFF=1` keeps the governor observing without pacing (`13-deployment.md`).
