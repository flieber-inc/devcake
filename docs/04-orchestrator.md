...
3. **Project auto-completion sweep:** for each Project carrying `DEVCAKE-TRACKING`, check its child Issues; if all are `done`/`canceled` (and it has ≥1 child), set the Project's status to `done`, remove `DEVCAKE-TRACKING`, and post a completion comment. (State is derived entirely from the PMO — no local tracking.) The child read is a flat in-project filter, so second-level decomposition composes correctly: a child canceled in favor of its own sub-missions counts terminal, while the sub-missions — created inside the same Project (`adr/0012`) — hold it open until they finish.
