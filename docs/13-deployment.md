...
Harness image tags follow **`DEVCAKE_TAG`** (same as app/admin — default `latest`): empty pin is `devcake/dev-*:${DEVCAKE_TAG}`; an explicit `cli_version` is `devcake/dev-*:${DEVCAKE_TAG}-${cli_version}` so two pins on one template cannot collide. Dispatch, steward, and OAuth go through **`resolve_image(dev_type)`** and **`require_staffed`**: a launch-supported template refuses unless `/data/harness_receipts` has an `ok` receipt whose digest equals the app's `DEVCAKE_APP_DIGEST` ARG. Hello stays `HELLO_IMAGE` and is not gated. Bare `bake app` leaves the sentinel `DEVCAKE_APP_DIGEST_UNSET`; `./up.sh --bake` computes `scripts/app_digest.py` and passes it to the app target and the host baker. Keep `DEVCAKE_TAG` exported for `compose up` so dispatch matches what you baked.

Which Dev image a run uses is `resolve_image(dev_type)` (`08-harness-templates.md` §2); `docker_image` is no longer stored config. The **host baker** (`scripts/dev_factory`, started by `./up.sh`, pidfile `.factory/watch.pid`) is the bake **and prune** verb: it reads the app-published keep-set, validates it, compiles, probes, and writes receipts (including a not-ok receipt — that is a finished bake for this digest, not a missing one) plus `/data/harness_bake_status.json`. A pin that already has a receipt for this app digest is not rebaked until the tree id moves. Operator ⋯ **Prune unused Dev images** writes `/data/harness_prune_request.json` (timestamp only; no image names). The baker independently deletes local `devcake/dev-*` images that are not in the keep-set, not used by a running container, and not hello. Anything else (including `nginx`) is refused. Receipts carry `scripts/app_digest.py` of **this checkout**; if that disagrees with the running app's `DEVCAKE_APP_DIGEST`, the baker does not bake and says so (`run ./up.sh --bake`). Each tick probes the app's `/health/live` the way `up.sh` does — three consecutive failures and the baker **exits**. The app can stay up if the baker dies; `/health` then reports `baker_alive: false` and the admin paints a red critical warning (same place as a circuit breaker) telling you to `./up.sh`. `dev-run.yaml` uses `pull: never` for every `devcake/dev-*` launch (audit A7). The app never talks to Docker. Dagu never bakes. A Claude-only keep-set does not grow Qwen.
