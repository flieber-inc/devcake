...
> **Status:** implementation in progress per [`docs/16-roadmap.md`](docs/16-roadmap.md) — **M5 complete** — the state machine is whole: missions now travel Backlog → ONBOARD → PLAN → EXECUTE → REVIEW → merged PR → Done fully autonomously, with skeptical reviews, merge-before-Done, reject/rework loops with cost warnings, decomposition with depth limits, and project auto-completion. Next: M6 (admin Config tab, GUI OAuth helpers, GitLab).
