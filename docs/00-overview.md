...
| **PMO System** | The external project-management system holding the Missions. Adapters are pluggable (registered in `adapters/registry.py`); in-tree: **Linear**, **Gitea Issues**, **GitHub Issues**, and **GitLab Issues**. A stack may configure **0..N** PMO instances; **each instance is scoped to exactly one team** (Linear team key, or Gitea `owner/repo` board). Accessed only through the `PMOPort` adapter (`05-pmo-adapter.md`). An instance also binds read-only context: reference repos and memory notebooks (`memory_repos`). |
