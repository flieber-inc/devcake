...
class MessagingPort(Protocol):
    ...
    # run_ids with ANY still-present ingress envelope (heartbeat/log/chunk/
    # artifacts — not artifacts-only). Watchdog finalize-stall backstop,
    # boot reconcile (docs/04 §6 / CAKE-73), and kill-race promotion: callers
    # that need "artifacts pending" must pair this with dagu_run_not_alive
    # (or an artifacts-scoped probe) — membership alone is not that signal.
