...
| `rev` | `int` (default 0) | Lost-update fence: bumped by `RunStore.save()` on every write so two writers holding different objects for the same run collide loudly instead of last-writer-wins silently. Legacy records parse as `0`. |
