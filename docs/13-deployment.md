...
> (**accepted**). Phase 1a ships the installable `devcake-cli` package
> (`cli/` → import `devcake_cli`; `uv tool install .` / `pipx install .`
> from this checkout or a git URL) whose first real verb is
> **`devcake baker run`** — the host baker entry under all three supervisors
> (systemd `--user`, launchd LaunchAgent, flock respawn) and
> `./up.sh --foreground-baker`. In-place upgrade: install/upgrade the CLI →
> re-run `./up.sh` (displaces leftover `python -m dev_factory` **and**
> `devcake baker run` children, rewrites units). Deprecated alias
> `python -m dev_factory` remains import-compatible during the transition
> (Decision 5). Sibling verbs (`up` / `down` / `status` / `doctor` / `bake` /
> `setup`) and the thin `./up.sh` → `devcake up` shim are later issues;
> until then `./up.sh` remains the live bring-up path (CLI `up` will
> auto-init missing `.env` bootstrap secrets by default — today's script
> still expects the operator to fill passwords after `cp .env.example .env`).
