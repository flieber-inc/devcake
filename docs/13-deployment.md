...
> (**accepted**; `devcake up` / `down` / `status` / `doctor` / `bake` /
> `baker run` / `setup`; clean-host chain `devcake up --bake &&
> devcake setup …`). Until that package ships, `./up.sh` remains the live
> bring-up path documented below (CLI `up` will auto-init missing `.env`
> bootstrap secrets by default — today's script still expects the operator
> to fill passwords after `cp .env.example .env`).
