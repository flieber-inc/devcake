...
| `POST /api/v1/system/clear-runs` | Operator wipe: stop in-flight Devs, delete local run records + audit log, purge Dagu run history, delete OpenObserve log/trace streams, **delete every `activity-*` repo on the internal Gitea** (ADR-0014 D4 — the PMO stays the source of truth; repo git history, incl. pre-edit feed states, is lost). Config + secrets + PMO + operator repos + skill-store + work repos untouched (`10-persistence.md` §5) |
...
- **Skills**: toggle-chip multi-select from the skill-store catalog (the shared `SelectionChips` control — see the multi-select convention above). Enabled whenever the selected harness declares a `skills_dir` in the registry (all three current harnesses do — `08-harness-templates.md` §7a); a harness without one renders the chips disabled with a hint and dispatch skips with a warning. A selected-but-missing skill renders as the standard red ✕ stale chip and is skipped at dispatch with a warning.
