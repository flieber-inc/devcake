...
- The Dev-side story is curl against Gitea's `/api/v1` (probe-verified `pr_instructions`), not an unverified `tea` CLI — no Dev-image rebuild, no speculative CLI flags. A `tea` CLI was considered as a recorded fallback and was **not** shipped in the Dev images; curl/`pr_instructions` is the only Dev-side Gitea path today.
