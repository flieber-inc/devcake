...
| `run.started` | Dev→app | `{container_hostname}` | Sent by the **provision** container after its `runspec.get` (ADR-0025 — the sole sender; the harness container relies on its heartbeat). App marks Run `running` and stamps `started_at` ONLY on the `dispatched→running` transition — every replay (redelivery, XADD retry, Dagu-UI re-run) is dropped, which also prevents a stray `run.started` from reverting a `finalizing` run or clobbering `started_at`. |
