...
- **Board side effects of `ensure_labels`:** the call best-effort PATCHes the repo to set `internal_tracker.enable_issue_dependencies: True` only (off by default on new repos; required for `blocked_by` / `create_relation`). It does **not** send `enable_time_tracker` or `allow_only_contributors_to_track_time` — the built-in time tracker is left alone. PATCH failures are logged and the ensure continues; `create_relation` may still fail until deps are on.
