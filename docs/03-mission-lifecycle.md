...
- **Finalization (app-side, always):** post the step comment (the Dev's last message, `>`-blockquoted) with the `{seq}_{TYPE}.md` full-session transcript attached (ADR-0014) → post token report (§8; threaded under the step comment where the vendor supports replies) → compare-and-transition (`04-orchestrator.md` §4) → forge side effects if any. **Exception:** on REVIEW-approve, the merge precedes the Done transition (§4.1) — Done must never overstate the repository.
...
### 1.1 Trivial path
ONBOARD never implements — it holds no write token whenever the recommended RO PAT is configured (per-stage least privilege, `14-security.md` §3/§8), and the old self-executing trivial path only ever worked on repo cards without one. Trivial is the degenerate case of the opportunistic plan (§1.2): the Dev writes the short exact plan to `/workspace/out/PLAN.md` and returns `outcome: "plan_needed"`; finalization takes the plan-attach path and the mission jumps straight to EXECUTE — the only stage that can push. `executed_trivially` is removed; a stray one parks with `DEVCAKE-SKIP` like any illegal outcome. REVIEW still always stands between any DevCake-written code and Done/merge — unchanged.
...
**Delivery destination (ADR-0017 addendum):** every mission's change set rides a pull request; what ONBOARD may declare is where that change set *lands*. `result.json` may carry `delivery_to: "repository"` (the default — merge, silent) or `delivery_to: "ticket"` with a one-line `delivery_reason`, strictly when no file in any repository should change and the deliverable is information for the ticket's readers. The declaration is record data the **app** writes: on a mission that carries no `` `devcake:delivery:v1` `` marker yet, a `ticket` declaration is appended to the description as `` `devcake:delivery:v1 to=ticket` `` plus a `Reason:` line (redacted, defanged, capped — `feed.marked_note` through the one append path `feed.append_note`), **before** the outcome's own labels, so no dispatch ever sees a stage label without the record. The plan comment (opportunistic plan) or a plain ℹ️ notice (bare `plan_needed`) states it in one sentence and names the line to edit. The park follows the board's `plan_approval`, like any plan. On a board whose PMO cannot hold attachments the ticket destination is unavailable: the declaration is forced to the repository and the comment says so. If the append fails (vendor description cap) the transition still completes — in repository mode — and the comment says exactly which line a person may add themselves; a run is never stranded over the note. **A later declaration is a proposal, never a write:** ONBOARD on a mission that already carries a marker (a decomposition child, a person's line) and EXECUTE may only *propose* a different destination — the run finalizes normally under the destination it was dispatched with (`Run.delivery_to`), then one `✋` notice states the proposal with the exact marker line to paste and `DEVCAKE-NEEDS-HUMAN` parks the mission regardless of `plan_approval`; the person's own edit of the description is the decision. A declaration equal to the record is a no-op. A malformed declaration (unknown value, `ticket` without a reason) is a structurally invalid payload → `DEV_BAD_OUTPUT` (§6). Last marker wins (`markers.delivery_of`), so a person reverts by writing a newer line. PLAN cannot declare (its result is entrypoint-synthesized); the `planned` gate comment restates the recorded destination.
...
- **Delivery destination per child (ADR-0017 addendum):** a draft may carry `delivery_to: "ticket"` with a one-line `delivery_reason` for a part whose deliverable is information for the ticket (a measurement, a report) rather than a repository change; the app stamps the child's footer marker itself (`02-domain-model.md` §11), and the child's own ONBOARD may only propose a change afterwards. Omit it for parts that change code.
- **Explicit priority** on every child (required field).
- **Routing (`repo`):** a cross-repository split names one repository per child in the draft's `repo` field — card name or URL slug, validated against the instance's work repositories — and the app stamps the child's routing footer itself; prose in the description never routes (`02-domain-model.md` §11).
...
- original is a **Project** → children are created inside the Project, the Project gets `DEVCAKE-TRACKING` and stays open; the poll loop auto-completes it when all children are done (`04-orchestrator.md` §1 step 3 / tracking sweep, ADR-0006). Projects always take this path — never trivial or normal (`05-pmo-adapter.md` §6). Relations are issue-scoped, so the project path needs no edge inheritance. Project ONBOARD runs always use **`seq = 1`** (projects have no issue-style comment feed to derive seq from — `04-orchestrator.md` / dispatch).
...
**A split is a plan.** The same toggle covers decomposition (§1.3): every child is created already carrying `DEVCAKE-NEEDS-HUMAN` (next to `DEVCAKE-CREATED`), so the fan-out — N ONBOARD runs, each free to decompose again — never schedules until a person has looked at the split. The parent's decomposition note (a project update for project originals) says so. Approval is per child and is the same gesture: remove the label, and the child derives as ONBOARD like any backlog Mission. Reviewing the split is done on the tickets themselves — titles, descriptions, priorities and relations are operator-owned data, so reshape a child in place, cancel the ones not wanted, then release the rest. **A split is edited, not redone:** the original is canceled in the children's favor, and re-opening it for a fresh triage is refused as a manifest mismatch (§1.3 replay rules), which is the intended behavior. Per-child plan approval then applies again once a child reaches PLAN — that is what "approve before code" means on a board that opted in.

**The Dev is told, and the person is shown.** The gate must never be something a Dev has to infer. While a board gates plans, the ONBOARD, PLAN and EXECUTE playbooks carry the `{plan_approval_rule}` placeholder, rendered by dispatch from three code-owned wordings: triage learns that attaching a plan (or splitting) parks the ticket automatically and that `human_needed` is never the way to ask for approval; planning learns to lead with a summary a reviewer can approve at a glance; execution learns that its plan was approved by a person, by construction, so it never looks for an approval comment. A custom planning-stage template without the placeholder is flagged on `/health` and at save time (`11-admin-panel.md`) whenever any PMO gates plans. On the other side, the approval request appended to the plan comment leads with what is being approved — the plan file and its outline (its headings, bounded) — and then the two actions, so the person never has to reverse-engineer the ask from a transcript. Both halves exist because of a field run where a careful triage returned `human_needed` to request approval and the human received a hand-off wall of text with no plan attached.

It reuses the hand-off label and its recovery path deliberately — no eleventh managed label, and `DEVCAKE-NEEDS-HUMAN` is not a stage label, so the redelivery checks (`04-orchestrator.md`) are untouched. It is **not** a hand-off (§4a): the run's outcome stays `planned` / `plan_needed` / `decomposed`, its verdict is a plain success, the hand-off loop guardrail never counts it, and the audit action is its own (`plan_approval_gate`). Every ONBOARD outcome now reaches a person before code — a gate with a hole for triage-attached plans or for splits is no gate.
...
5. **Hard rule — Devs never merge, never approve on the forge, never push the default branch.** Opening or updating the mission branch and PR is the boundary; merging belongs to the app (`auto_merge`) or a human — lifecycle gates and receipts live above the Dev. Forge **branch protection** (PR + ≥1 approval + required status checks) is a **deployment requirement** for work repos (`14-security.md` §2 zone C, `13-deployment.md` §8a); required checks alone do not stop a patient Dev self-merge after green CI — playbooks state the never-merge rule in advance as the operative belt until required reviews + a distinct reviewer token are in place, and an unprotected default branch is loudly flagged on Overview via `/health` `forge_protection`.

`result.json`: `outcome: "executed"` with `pr_url` and `summary`. An EXECUTE runs under the destination it was dispatched with (`Run.delivery_to`, §1.2) — the pull request is required either way — and may add `delivery_to`/`delivery_reason` only to **propose** a different destination (a human comment asked for it, or the recorded one is impossible for the brief): the transition still swaps to REVIEW, then one `✋` notice states the proposal with the marker line to paste and `DEVCAKE-NEEDS-HUMAN` parks the mission until a person decides.
...
The Dev must: check out the PR branch in its clone; diff against the plan; hunt for bugs, flaws, and omissions; run the tests if present. The playbook prompt forbids rubber-stamping — the default posture is distrust. The same never-merge / never-forge-approve / never-push-default hard rule as EXECUTE applies: the REVIEW verdict lives in `result.json` only; formal forge approval and merge are app side effects. REVIEW never proposes a delivery destination (§1.2): a reviewer who finds the recorded destination wrong — a human comment asked for something else, code changed in a ticket-destination pull request, a report about to merge into a code repository — rejects and says so in the report; a `delivery_to` on a `reviewed` result is ignored with an audit line (or a human) — lifecycle gates and receipts live above the Dev.

`result.json`: `outcome: "reviewed"`, `verdict: "approve" | "reject"`, `report_md` (the full review report), `pr_url`.

### 4.1 Approve

**The Freshness Gate runs first (ADR-0031).** A positive-verdict REVIEW is the pipeline's last feed read, so before ANY approval artifact the app checks whether **material** feed entries arrived after the run's dispatch watermark (its reading receipt, `Run.feed_watermark`). Material = non-empty unquoted body that is either sentinel-less (🧑 HUMAN, §8a — steering posts included) or carries an explicitly **elevated** marker class (`ELEVATED_MARKERS` in `markers.py` — first member `` `devcake:discovery-in:v1` `` per ADR-0033; the source-side `` `devcake:discovery:v1` `` is deliberately not elevated so a mission's own harvest cannot trip its own gate); everything else DevCake posts is bookkeeping and immaterial by construction, which is what guarantees the loop terminates. A truncated fetch is material-UNKNOWN and trips (`Activity.truncated` — full-history hard stops are adapter-specific in *which* end they drop, but unknown is never treated as clean). On a trip the done-transition is **withheld**: a 🔄 directive carrying `` `devcake:freshness-rereview:N` `` posts (🔄 is reserved for this directive), the mission keeps `DEVCAKE-REVIEW`, and the next poll re-dispatches a REVIEW whose only job is judging whether the newer entries change the verdict. Counting is marker-derived like conflict-resolve (max over unquoted bodies); the lifetime **bound** is `AppConfig.budgets.freshness_rereviews` (default **5**, `0` = unlimited — ADR-0033 D7 as amended; conflict-resolve stays separate at `MAX_CONFLICT_RESOLVES = 2`). A human deleting the directives deliberately resets the count, and a human *pasting* one unquoted inflates it toward exhaustion: inherited doctrine, humans own the feed. Past the budget the standing verdict proceeds with an explicit ⚠ unevaluated-activity disclosure + audit event (the durable record — the `/health` anomaly entry is transient once the mission is done). A freshness re-review is **not a failure retry**: a finished run is an attempt-reset anchor, so it never consumes `attempt_of_step` nor feeds the ADR-0026 brakes (`15-errors-and-retries.md` §2). The check itself is fail-open (a gate error proceeds); the directive post, once material is found, is not — a failed post withholds the transition and lets the next plain REVIEW read the material from its mirror. The directive and the disclosure are notices (§4c): the directive opens `🔄 **Freshness re-review N/cap.**`, the disclosure `⚠️ **For the record.**`, and the counted marker rides the fold's `Record` section only — never the head, which would be a second count.
...
**Merge always precedes Done** — a Mission's Done status must never claim more than the repository shows (confirmed decision). The one other Done is the ticket delivery (§1.2): its files are on the ticket *before* Done, its pull request is closed unmerged *after*, and its copy says in so many words that no repository changed. Finalization: transcript + token report → forge effects, then the PMO transition:
...
3. **Ticket destination first** (ADR-0017 addendum, §1.2): when the mission's recorded destination is the ticket and the pull request is visible, the approve ends here — the pull request's changed files are attached to the ticket (each file its own attachment when the set is small and every file fits the PMO cap; otherwise the archive with its MANIFEST, exactly as the post-merge path builds it), posted as an ℹ️ notice carrying the `Deliverable` record **before** Done; then Done through the completion chokepoint (label `DEVCAKE-REVIEW` off, status `done`, a notice whose every word says *no repository changed* — a ticket Done never claims a merge); then the pull request is **closed without merging** (`ForgePort.close_pr`, best-effort *after* Done: a person is told when it stays open, and told not to merge it). No formal forge approval and no approval footer are posted — nothing should invite a merge. A delivery failure raises: the run is not done, `DEVCAKE-REVIEW` stays, and redelivery retries the checkpointed delivery. A ticket-destination approve whose pull request is not visible takes the missing-PR path below. Otherwise, by the **mission's repo** `auto_merge` (per-repo doctrine, ADR-0020; internal/zero-repo instances always ON):
   ...
   - **OFF:** remove `DEVCAKE-REVIEW`, add `DEVCAKE-MERGE`. The Mission stays In Progress; the **app** does not call merge.
   - **Pull request not visible at finalize** (forge list lag, another branch name, never opened) — every repo, ON or OFF: remove `DEVCAKE-REVIEW`, add `DEVCAKE-MERGE` and open the deferred window with `` `devcake:merge-retry` ``, naming the **branch** (never a placeholder for a url DevCake cannot name). ON: the sweep merges the moment the pull request appears. OFF: the record says merging it is the person's once it appears, and the sweep posts one notice naming it when it does. When `merge_retry_window_minutes` is 0 the hand-back is immediate: a `✋` notice carrying `` `devcake:merge-handoff` `` — "no pull request exists on branch …; open one or cancel the ticket" — and the awaiting-human-merge banner. The intended waiter is a human (awaiting-human-merge banner). Operator **Re-check freshness** (admin Missions ⋯, `force_freshness`) may re-open REVIEW if material arrived after the last REVIEW watermark. This toggle does **not** prevent a Dev with a write token from merging on the forge if branch protection allows — that residual is `14-security.md` §2 zone C.

**Every one of these posts is a notice (§4c, ADR-0042 §4).** The ⏳ settle and deferred-retry lines, the ✅ awaiting-merge line, the ⚠️ hand-off, out-of-pipeline and loop-warning lines and the 🧩 directive open with a fixed lead — `✋ **Needs you.**` when a person must act (the merge is theirs; the §5 command is the head's what-to-do), `⚠️ **For the record.**` for a disclosure, `ℹ️` for a status note, `🧩 **Conflict-resolve directive.**` for the next Dev — then one sentence for the reader; the comment text of the pre-notice build, its merge-state marker or counter included, is the fold's `Record` section, so the sweep's marker reads and the Dev's folder are unchanged.

**Merge sweep** (every poll cycle, alongside the `DEVCAKE-TRACKING` sweep — `04-orchestrator.md` §1): for each Mission carrying `DEVCAKE-MERGE` that is **not** terminal (`done`/`canceled`) — including forge-issue `backlog` (GitHub/Gitea Issues map open → backlog, docs/05 §9.2) — check its PR via the forge adapter (the branch→PR lookup is memoized per parked mission; a terminal answer on a memoized number is confirmed by a live lookup before anything is written) — merged → status `done` **then** remove the label (status is the derive commit; a failed status leaves the label so the next cycle still selects); closed without merging → cancel **then** remove the label, with a notice either way — an ℹ️ completion notice (the deliverable-archive note is appended to its fold as a `Deliverable` section, §4c; its own ℹ️ notice only when that edit is impossible) or a ⚠️ cancellation notice. Latest merge-state marker among `` `devcake:merge-settle` `` / `` `devcake:merge-retry` `` / `` `devcake:merge-handoff` `` selects the driver: **settle** — wait `merge_settle_minutes` then recheck freshness (may re-open REVIEW) before opening retry/merge; **retry** — deferred-merge window (`mergeable()` each cycle; conflict rework; once `merge_retry_window_minutes` have passed since the marker PMO timestamp the sweep keeps probing and merges when the forge says ready; the second probe in a row past the window that does not merge hands off — a cycle whose reads the request budget refused never costs the window its attempt, and one transient forge error is not terminal). A human can merge manually at any point mid-window. **`to=ticket` on a parked pull request:** a person who writes `` `devcake:delivery:v1 to=ticket` `` into the description of a mission parked at `DEVCAKE-MERGE` ("do not merge this, put it on the ticket") is honoured on the next cycle — files to the ticket, Done through the completion chokepoint (disclose-only freshness, as for the deferred merge), the pull request closed unmerged; a delivery failure hands back once (`` `devcake:merge-handoff` ``, latched for the process) and the mission stays parked. **No pull request on the branch:** inside the deferred window the wait is a visible `blocked_reasons` line (`/health`, admin panel); once the window has elapsed — or when no window was ever opened (a legacy or hand-edited park) — the sweep hands back **once** with `` `devcake:merge-handoff` `` naming the branch ("open a pull request on that branch, or cancel the ticket") and keeps the awaiting-human-merge banner; DevCake never cancels a mission for a missing pull request. State remains fully derivable from PMO + forge; nothing local. **Freshness at a pure deferred-retry close is still disclose-only (ADR-0031)** when the settle recheck already passed or settle was 0; settle-end is a full recheck that can trip re-review rather than disclose-and-merge over unread elevated material.
...
**Finalization:** transcript + token report → add `DEVCAKE-NEEDS-HUMAN` (the stage label stays, so work resumes at the same step) → post the baton as a `✋ **Needs you.**` notice (§4c): the Dev's summary blockquoted in the head, the resume instruction as the what-to-do, and the pre-notice baton text — summary included — as the fold's `Record` section, so the Dev's folder still carries the baton it always did. If the run was an ONBOARD (no stage label at dispatch) the status is restored to `backlog` — otherwise removing the label later would land on derivation row 9 and strand the Mission. For **project-kind** missions every feed write — the step card, this baton, the decomposition note — goes out as a **project update** through the same chokepoint (the vendor's project-native feed, sentinel-signed; `05-pmo-adapter.md` §6, ADR-0043 §2), so a project's record is as complete as an issue's.

**Loop guardrail (warnings only — never auto-park):** the app counts prior `human_needed` runs for the same (mission, stage) from the run store; from the 2nd hand-off on, the baton's record carries an escalating header — "Hand-off #N on this step … add `DEVCAKE-SKIP` to stop DevCake on it." — and the notice head says so in its own words. DevCake never parks on its own; the human always decides.
...
**The release is visible to the Dev (a resume is a human's answer).** Only a person removes `DEVCAKE-NEEDS-HUMAN`, but a label removal is not a feed entry, so the next run could not see that it had been released and — when its brief expected an approval — asked again. Now, when a mission's most recent finished run was a genuine hand-off, the next run's `MISSION.md` opens with a one-line pointer and its activity mirror with a `▶ RESUMED BY A HUMAN` banner: which step handed off, when, the exact ask, and how many 🧑 HUMAN comments that run never saw — its feed watermark is the boundary, so an answer posted while it was still running counts (`07-dev-runtime.md` §2). Only a hold qualifies: a run halted by an external stage change, or parked for an illegal outcome, never triggers it, and ancestor mirrors never carry it. The playbooks state the convention next to the hand-off instruction: a human comment that run never saw is the answer; without one, the release itself is the answer — an approval asked for is granted, a choice goes to the option the Dev recommended, something asked to be provided should now be there and is verified — the Dev re-reads the brief in case it was revised instead of commented, and hands off again only if the obstacle demonstrably persists. A brief therefore never needs to invent its own approval protocol. On a metered tracker, release parked missions in batches: every release is a dispatch, and dozens at once spend the credential's hour on write-backs (`05` §2a).

## 4b. Relations Steward (`STEWARD` runs)

> **Naming:** STEWARD (formerly MAPPER) is the general class of out-of-the-loop, board-tending Dev runs; relations mapping is its first duty; discovery routing (ADR-0033) is its second — see the DISCOVERY-flavor block below. Live code, config, records, and docs use STEWARD; one-time migrations rewrote persisted config (`relations_mapper` → `steward`, dev type `mapper` → `steward`), settings bundles, and run records. Pre-rename ADRs and historical feed comments/run ids keep the old name as the immutable record.

A **team-scoped run kind** (not a Mission Type — it has no host Mission and no labels) whose only job is proposing missing blocked-by relations across the team's open Missions. Configured under `AppConfig.steward` (`02-domain-model.md` §9): **manual-only by default** (the admin "Run now" button) with an opt-in periodic service. Its usual vehicle is the operator-staffed **steward** Dev Type from first-setup (or an equivalent Dev Type the operator names in `steward.dev_type` — ADR-0033 D10: the steward class carries an EXECUTE-grade bar for BOTH duties, because a wrong blocked-by edge silently reorders a family's execution and discovery routing is family-wide relevance judgment). Fresh installs ship no seeded steward and no vendor model pin; harness and model are operator-chosen. The repo clone is load-bearing for the discovery flavor: evidence anchors are paths, and a steward that can grep the tree judges scope against ground truth.

**Cadence & degradation (`StewardService`):** one lock serializes the manual and periodic paths (no double dispatch); the interval watermark advances only after a successful dispatch (a transient executor error costs one poll cycle, not a full interval); and when this board's 3 most recent STEWARD runs all died, the periodic service and the discovery drain **back off** for three steward intervals from the newest death and then admit one run — surfaced as `steward_degraded` in `/health` and on the admin card while paused — while "Run now" stays available throughout; a further death re-arms the pause, a successful run clears the condition (store-derived, restart-safe, per instance).
...
- **Finalization (the app is the gatekeeper):** each proposed edge is validated against a live snapshot and dropped (audited `steward_edge_rejected`) if it references an unknown or terminal key, is a self-edge, duplicates an existing relation, or would create a cycle in the blocked-by graph. Surviving edges become native PMO relations, and the blocked Mission gets an ℹ️ notice naming its blocker ("delete the relation in the PMO if wrong"). No PMO feed transcript/token-report posts (INV-5 named exception — there is no host Mission); storing transcripts as logs in the corresponding Gitea work repositories is acceptable. Failures are logged only and the next interval retries.

**The DISCOVERY flavor (ADR-0033).** The steward class's second duty, on the same run kind (`Run.steward_duty = "discovery"`; same `stewarded` outcome). Trigger: event-kicked when harvest memorializes a run's `discoveries`, re-driven by the label-gated sweep; one family's pending batches per run, per-instance single-flight, gated by `pmos[].discovery_routing`. Context is **curated, not accumulated**: the family map (decomposition tree ∪ blocked-by component, statuses included), finished members' handoffs, open members' description heads, the new discovery entries at full fidelity, and the family's work repos cloned read-only (evidence anchoring; clone-capped like blocker mounts). Output is propose-only routes — `{"routes": [{"target", "source", "step", "finding", "because"}], "declined": […]}` — and the app is again the gatekeeper: targets must be family members and non-terminal, `(source, step)` pairs already on a recipient are never re-delivered (this dedup — not a numeric budget — is the fan-out bound, addendum 14), and **finding text is copied from the source run record, never from steward output** (verbatim transport is structural). Delivery is one **leads notice** per recipient — `📨 **Leads from KEY, step N.**`, the findings quoted in the head, and the marked delivery text (the elevated `` `devcake:discovery-in:v1 src=<KEY> step=<n>` `` marker, the finding fingerprints, the findings) as the fold's `Record` (elevated: it trips an in-flight recipient's freshness re-review; over the inline budget the findings are dropped from head and record alike, the marker lines never) — plus a MISSION.md closing block ("leads, not truths"); every dispatched batch is receipted on the source by **appending a `Routing receipts` section to the fold of the step card that harvested it** (`` `devcake:discovery-routed:v1 step=<n> to=<KEY|->` `` — `-` = deliberately routed nowhere; ADR-0042 §6: an edit of DevCake's own card, never a new entry — when the card is missing, paged, over the vendor cap or the edit fails, the receipt text posts as a ⚠️ notice whose `Record` it is, audited `discovery_receipt_fold_failed` unless there was no card, so `pending = posted − receipted` always closes), and the sweep-gate label drops once nothing is pending. A source run record that exists but is not yet terminal is **in flight** (the harvest posts the marker before the close finishes): the sweep holds such a batch — label and pending id kept, nothing posted — and reserves the unroutable `to=-` disposition (appended to the card's fold the same way, else a ⚠️ notice) for a record that is absent or terminal without a usable result; the close also writes the run's `result` onto the record before the harvest posts, so the window is closed from both sides.
...
was appended later (routing receipts, the deliverable note). Every backticked marker a scan reads
...
**Notices — anything written to a person (ADR-0042 §4).** One shape at
every site: a fixed lead — `✋ **Needs you.**` when a person must act,
...
never carries a backticked marker, a step-file token or a `Part i of n`
line (a second copy would double-count a set-valued scan); model text in
the head (a hand-off summary, the leads) is `>`-blockquoted. The projection
emits the `Record` as the Dev's entry, so a notice, like a card, changes
nothing a Dev reads. Late bookkeeping that belongs to an existing entry —
routing receipts on the harvesting card, the deliverable note on the
completion notice — is appended to that entry's fold by edit and posts as
a notice of its own only when the edit is impossible.
...
```jsonc
{
  ...
  "decomposition": [                       // ONBOARD 'decomposed' only
    {"title": "…", "description": "…", "priority": "high",
     ...
     "delivery_to": "ticket",              // optional: the child's destination from birth (§1.3)
     "delivery_reason": "…one line…",      // required with delivery_to: "ticket"
  ...
  "delivery_to": "repository | ticket",   // optional — ONBOARD plan_needed (declares; §1.2), EXECUTE (proposes; §3)
  "delivery_reason": "…one line…",        // required with "ticket"; ignored on reviewed
  "discoveries": [                         // optional — ONBOARD/EXECUTE/REVIEW (ADR-0033)
    {"finding": "…stated for a stranger…",
     ...
     "about": ["…optional topic tags…"]}   // optional list[str]; non-strings / missing → empty
  ]
}
```
...
The full prompt a Dev receives = **identifying prompt** (Dev Type, below) + **playbook prompt** (per Mission Type, maintained as templates in `app/devcake/prompts/`, interpolated with mission metadata; the EXECUTE playbook additionally interpolates the forge descriptor's `pr_instructions`) + **optional required-skills soft-force block** when `DevType.skills_required` is non-empty and those skills shipped in the runspec (`02-domain-model.md` §6; **ADR-0016**). Skill *files* are installed separately into the harness skills dir (consult-optional by default). Playbook prompts inline only the mission title and description; the `activity/` folder is presented as **reference material to consult as needed** ("the mission's history and artifacts are in activity/ — grep or read what you need"), never dumped into the prompt (`07-dev-runtime.md` §2). The playbook prompts restate, verbatim, the binding rules from this document: workspace boundaries (INV-6), commit-at-end, branch conventions, the Dev never-merge / never-forge-approve / never-push-default boundary (§§3–4), the standalone rule, the depth limit, the `result.json` contract, and the **delivery-destination contract** (§1.2, ADR-0017 addendum — a code-owned epilogue on all four playbooks: ONBOARD on a fresh mission may declare `delivery_to`; everyone else reads the recorded destination and may at most propose; REVIEW never proposes; the epilogue names when ticket delivery is unavailable on the board) — including, for the three result-authoring types, the ADR-0033 discoveries contract (a code-owned epilogue like the human-handoff exit, so operator template overrides keep it). Skills never replace playbooks (**ADR-0016**, `app/devcake/skills/README.md`).

### Judge — identifying prompt
> You are **Judge**, DevCake's judgment-heavy engineer. You assess, plan, and review software work with the skepticism of a staff engineer who has been burned before. You are precise about scope: you do exactly what your current mission playbook asks — no more. You never invent requirements, you flag what you cannot verify, and you write conclusions that a teammate can act on without asking follow-up questions.

### Executor — identifying prompt
> You are **Executor**, DevCake's implementation engineer. You turn plans into working, tested code. You follow the plan you are given; where reality contradicts the plan, you implement the smallest sound deviation and document it prominently in your summary. You match the conventions of the codebase you are in, you run the tests, and you never commit until the work is complete. Do exactly what your current mission playbook asks.
...
Posted to the activity feed immediately after each transcript on the issue mission-step path (INV-5; see named exceptions for truly FAILED / STEWARD / CURATOR):
