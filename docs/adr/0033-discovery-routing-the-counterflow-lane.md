...
2. **Content dedup at delivery.** Every delivered finding carries a per-finding fingerprint line (`` `devcake:finding:v1 sha=…` ``, twelve hex characters of the case-folded, whitespace-collapsed finding text). Before delivering, the apply step reads the recipient's fingerprints (any source) and skips a finding already on the recipient, auditing `discovery_route_duplicate_content`; the receipt still records the route, so the batch dispositions. The pair dedup of Decision 6 bounds the same batch; this bounds the same finding from many sources. Not a budget — the second bound the ADR always named.
