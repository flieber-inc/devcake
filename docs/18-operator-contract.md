...
| **Clear runs as a maintenance window** | When you use it | Clear-runs holds the poll + dispatch locks for the whole wipe (seconds, longer if a Dev must be drained): no new run dispatches until it finishes, and any Dev still live is stopped-then-drained first. `ok: false` with `undrained` means a container could not be stopped (the app has no `docker.sock`) — investigate before re-running work (`13` §8) |
