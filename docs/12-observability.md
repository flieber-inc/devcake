...
- The **host baker** is not a container. It appends structured records to `/data/harness_baker.jsonl`; the app poll cycle ships them through **`push_oo_log("baker", …)`** (the same chokepoint as `run_failures`). If the app is already down, the baker POSTs one dying word to OO on loopback with the ingest account — it does not go through the collector (a sick collector must never blind this either). `service.name` for baker transition spans is **`devcake-app`** (the app observes; the host process does not install its own tracer).
