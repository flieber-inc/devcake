...
| 16 | Harness stopped at its configured turn cap (`--max-turns`) — **`claude-code` only today**, see below | `DEV_TURN_BUDGET` (always counted; deterministic, so never correlated) |
...
**Which harnesses can reach exit 16 (measured 2026-07-25, `08-harness-templates.md` §1b).** Only `claude-code`: its cap exhaustion surfaces as `terminal_reason: "max_turns"` / `subtype: "error_max_turns"`, and that is the sole turn-budget arm the predicate has. `grok-build` also takes `--max-turns` and does stop on it — at 0.2.112 it emits `max_turns_reached` **and** an `end` with `stopReason: "Cancelled"`, exiting 1 — but no grok arm reads those events, so a grok cap stop is classified `DEV_CRASH` (exit 10) today, not exit 16. `codex` at 0.144.4 has **no `--max-turns` equivalent and no config key for one**, so exit 16 is unreachable for it and no value of the per-Mission-Type extra CLI args (`02-domain-model.md` §9) can bound it.
