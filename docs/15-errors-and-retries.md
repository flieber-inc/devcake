...
|---|---|---|
| `PMO_TRANSIENT` | vendor rate limit (429 / an in-band `RATELIMITED` error), 5xx, network — adapters signal it by raising `PMOTransient` (`ports/pmo.py`); the request budget's own refusal, made BEFORE any vendor call, is the `PMOBudgetExceeded` subclass (ADR-0040) | retryable |
...
|---|---|---|---|---|
| `PMO_TRANSIENT` | yes — **no per-vendor retry ladder.** One **governed** retry lives in the shared request budget (ADR-0040, `05-pmo-adapter.md` §2a): a **critical**-class call (finalize, dispatch, operator actions) waits for the vendor's refill under a cumulative per-context deadline and is retried ONCE after a definitive rejection (a 429/RATELIMITED response was never executed); network errors are never retried. **Routine**-class calls (the poll's reads, sweeps, probes) fail fast — the adapter raises `PMOTransient` / the budget raises `PMOBudgetExceeded` — and the poll cycle **skips that PMO instance's segment**, continuing with the others; the next poll tick re-attempts the instance | no | budget (critical) / app (next poll tick) | `poll.instance` outcome / logs (with the vendor's retry-after hint when known); `/health.pmo_budget`; not latched into `poll_degraded` (that field is for permanent per-instance failures) |
