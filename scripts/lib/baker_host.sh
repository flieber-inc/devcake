...
devcake_baker_resolve_cli() {
  ...
  bin="$(command -v devcake)" || {
    ...
    echo "   then re-run ./up.sh (ADR-0038 Decision 5/6)" >&2
    return 1
  }
  printf '%s' "$bin"
}
...
devcake_baker_write_runner() {
  ...
  local runner devcake_bin pythonpath
  runner="${factory_dir}/baker-run.sh"
  devcake_bin="$(devcake_baker_resolve_cli)" || return 1
  pythonpath="${repo}/scripts:${repo}/app"
  mkdir -p "$factory_dir" || return 1
  cat >"$runner" <<EOF
#!/usr/bin/env bash
# Generated by ./up.sh — sources baker.env then execs \`devcake baker run\`.
set -euo pipefail
cd "${repo}"
