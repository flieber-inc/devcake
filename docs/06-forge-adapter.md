...
- `ProtectionShape` — apply/compare shape: `{require_pull_request, allow_force_push, allow_deletions, required_status_checks: list[str], required_approving_review_count, require_status_checks_unscoped: bool}`. Check names are discovered from the target repo (empty when no CI) — never hardcoded. `require_status_checks_unscoped` marks a forge-level CI gate without named contexts (GitLab Free `only_allow_merge_if_pipeline_succeeds`; Gitea `enable_status_check` with empty contexts or only `"*"`). It is never treated as a derived check name — Gitea writes `"*"` only as the vendor wire encoding of that flag.
...
- `PRFilesResult` — `{files: list[PRFile], truncated: bool}`. `truncated=True` means the vendor withheld some changed paths (names unknown). GitLab walks the paginated `/merge_requests/{iid}/diffs` (100 per page) and, like Gitea, passes through `paginate_rest`'s ceiling flag (forty pages ⇒ `truncated=True`), and also reads the merge request's `changes_count` — a trailing `+` is the vendor's own cap ⇒ `truncated=True`; a `too_large`/`collapsed` entry withholds only the diff text, never the path. GitHub paginates to completion (`truncated=False`). Delivery discloses residual truncation in `MANIFEST.txt` and the feed note without inventing dropped filenames.
...
| No-weaken | If existing protection already meets or exceeds the desired shape → `outcome="already_as_strict"` (no write). Writes merge strictest-of(current, desired). Vendor writes must not drop unmodeled stricter settings (GitHub classic PUT preserves code-owners / restrictions / linear-history / conversation-resolution; GitLab delete+recreate restores the prior rule if recreate fails; Gitea maps unscoped CI onto `require_status_checks_unscoped` and preserves it on write via vendor `"*"` — never clears an existing status-check gate when tightening another dimension). |
...
- **Deferred** (`True`/`None` — ready-but-raced, mergeability computing, CI pending): the Mission lands on `DEVCAKE-MERGE` and the merge sweep retries for that repo's `merge_retry_window_minutes`, keeps probing once the window has passed (and merges when the forge says ready), and hands off after the second probe in a row that does not merge — one transient forge error is not terminal. On large repos mergeability computation takes a while and CI-gated repos legitimately block merges for many minutes — `None` means "keep watching," never "give up."
...
- `apply_default_branch_protection` writes **classic** protection via `PUT /branches/{branch}/protection` (admin / edit-repository-rules). The PUT body is merged from any existing classic rule so unmodeled stricter fields (code-owner reviews, push restrictions, linear history, conversation resolution, …) are preserved — GitHub replaces the whole rule. Rulesets remain a read signal only on this path. Two fixed posture choices on write: admin enforcement is always enabled (the Dev's write identity must not bypass — docs/14 §2 zone C), and the up-to-date-before-merge flag (`strict`) is preserved from the existing rule and never newly enabled — it is not part of the derived shape.
...
- `apply_default_branch_protection` (re)creates `/protected_branches` with `push_access_level=0` and `allow_force_push=false`, then sets project `approvals_before_merge` / `only_allow_merge_if_pipeline_succeeds` to match the derived shape (GitLab Free has no per-context required-check list — as-strict uses `require_status_checks_unscoped`, never a `"*pipeline*"` sentinel). If delete+recreate is used, a failed POST/PUT after DELETE restores the captured prior rule before raising.
- **`pr_files` listing:** `GET /merge_requests/{iid}/diffs?per_page=100&page=N`, walked to completion (the `/changes` endpoint is deprecated since GitLab 15.7 and leaves in API v5; its `access_raw_diffs` retry read whole diffs from Gitaly with no size cap). `too_large` / `collapsed` entries keep their path. Past the forty-page ceiling, or when the merge request itself reports `changes_count: "1000+"` (GitLab stopped storing the diff at its cap — a signal the page walk cannot see, so the adapter reads the merge request row too), `PRFilesResult.truncated=True` and delivery discloses that additional paths are unknown (MANIFEST + feed note), pointing at the MR as canonical.
...
- `apply_default_branch_protection` POSTs/PATCHes `/branch_protections` with `enable_push=false`, `enable_force_push=false`, discovered `status_check_contexts`, and `required_approvals` from the distinct-reviewer rule. An existing rule with `enable_status_check=true` and empty contexts (or only `"*"`) maps to `require_status_checks_unscoped=True` — not a fake check name. Writes keep that gate (`enable_status_check=true` + vendor `"*"` when no named contexts); discovered named contexts replace `"*"` when present. Distinct from the provisioner's `_ensure_protection` (always approvals=1 + `devcake-reviewer` whitelist — internal repos only).
