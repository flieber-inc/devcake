...
- **(d) A reliable activity feed.** Ordered comments + file attachments with **markdown fidelity** for backticked markers (`devcake:v1`, decomposition manifests, merge-retry markers): the feed is DevCake's persistent record of each mission (long-lived cross-mission memory is the separate notebook system, ADR-0035). A PMO that rewrites comment bytes (ADF/rich-text — Jira) needs an explicit fidelity strategy **on the port** before an adapter is attempted (ISSUES #35; strategies `raw` / `transcoded` / `sidecar` — `docs/16-roadmap.md` launch-roster note). Do not start a Jira adapter that greps ADF for markers. Abandonment must be expressible (`cancel_mission` — a canceled/archived terminal state). A vendor with no official attachment upload is not silently “supported” — either the live contract row 13 passes, or `attachments_supported` is declared false and docs/05 §0 (d) is updated in the same change (GitHub Issues spike, 2026-08-15).
