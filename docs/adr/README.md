...
| [0040](0040-pmo-request-budget.md) | PMO request budget: one governor, vendor headers stay in adapters | Accepted (2026-09-03); amends ADR-0034 (one governed retry in one place) and retires ADR-0003's "rate limits are comfortable"; two call classes + reserve, transient finalize failures never poison |
| [0041](0041-operator-mcp-server.md) | Operator MCP server: the API's own description is the tool catalogue | Accepted (2026-09-06) — `devcake mcp` shipped; phase two (a read-only principal) deferred to a debate |
| [0042](0042-feed-for-readers.md) | The feed for readers: step cards, one fold for the record, one status comment | Accepted (2026-09-09) — the Dev folder is a projection of the record, held by a golden test |
| [0043](0043-activity-repository-is-the-record.md) | The activity repository is the record of a mission's run | Accepted (2026-09-10), delivered in stages — refreshed at every run boundary; project runs write their record; upstream folders (ancestors, project, direct done blockers) are copies of it; reverses ADR-0036's rejected option (b) |
| [0044](0044-deadlines-never-cancel-a-socket-holder.md) | Deadlines never cancel a socket holder | Accepted (2026-09-22) — `devcake.deadline` is the one way to bound a wait; probes say `pending`; a structure guard bans `asyncio.timeout`/`wait_for` around HTTP; `/health.http_pool` meters the leak class |
