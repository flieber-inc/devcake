...
4. **One resolver** (`RepoCache.resolved_branch`: pin, else the mirror's HEAD when its target exists, never a bare-init HEAD) feeds the Dev's env and playbook, the branch-protection probe and apply, and the claims writer; an unresolved blank card defers dispatch and refuses protection apply rather than render an empty name. The health protection probe covers work repos only (cards a board lists under `repos`).
