...
| **Dev Type** | A named configuration: harness template + model selection + identifying prompt + MCP servers + credentials + concurrency cap + optional domain skills + optional domain-bound memory notebooks (`memory_repos`). v0 seeds three role vehicles: **judgment** (Claude Fable / Claude Code — ONBOARD/PLAN/REVIEW), **implementer** (Grok 4.5 / Grok Build — EXECUTE), and **steward** (Claude Opus / Claude Code — the board-tending class: Relations + Discovery Steward; EXECUTE-grade bar per ADR-0033 D10). The model belongs to the Dev Type; multiple Dev Types may share one harness. |
| **Harness Template** | A registry-backed runtime adapter. Launch-supported: `claude-code`, `grok-build`, `codex`. **Experimental** (in-tree, not launch-supported): `pi`, `opencode`, `qwen-code`. A template selects the CLI image, credential/OAuth contract, invocation and artifact parsing, and skills directory — not a fixed model. `DevType.model` selects the model; empty uses the registry or CLI default. Specified in `08-harness-templates.md`. |
...
| **PMO System** | The external project-management system holding the Missions. Adapters are pluggable (registered in `adapters/registry.py`). Launch-supported: **Linear**, **Gitea Issues**. **Experimental** (in-tree, not launch-supported): **GitHub Issues**, **GitLab Issues**. A stack may configure **0..N** PMO instances; **each instance is scoped to exactly one team** (Linear team key, or Gitea / GitHub / GitLab `owner/repo` board). Accessed only through the `PMOPort` adapter (`05-pmo-adapter.md`). An instance also binds read-only context: reference repos and memory notebooks (`memory_repos`). |
...
4. **Dagu spawns the run's two containers.** The **provision** container (`prov-<run_id>`) mounts the source mirrors read-only + this run's workspace, prepares `/workspace/repo` (fresh clone), `/workspace/activity/ACTIVITY.md` (+ attachments), any bound memory notebooks at `/workspace/memory/<card>/` (read-only), and exits; then the **harness** container (`dev-<run_id>`) mounts ONLY the workspace — never the mirrors — registers MCP servers, and launches Claude Code with the judgment identifying prompt + the ONBOARD playbook prompt (ADR-0025; `07-dev-runtime.md`, `08-harness-templates.md`, `03-mission-lifecycle.md`).
