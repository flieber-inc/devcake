...
| Field | Type | Notes |
|---|---|---|
| `name` | `str` | e.g. `judge`, `executor`, `steward` (kebab-case slug; display name derived). Operator-chosen — fresh installs ship no seeded names. |
...
| `model` | `str` | Pins the harness model. Delivered via runspec as `DEVCAKE_MODEL`; the entrypoint maps it to the harness flag (`claude --model` / `codex -m` / `grok --model`). Empty = harness default. No vendor model name ships in the seed path — the first-setup wizard leaves model blank unless the operator fills it. Per-assignment `extra_cli_args` can still override (appended after the pin). |
...
**First boot:** the Dev Type roster starts **empty** — no vendor model names and no house Dev Types are written under `/data/config/dev_types/`. When the roster is empty, Fleet → Dev Types and Overview offer a first-setup wizard (`POST /api/v1/dev-types/first-setup`) that creates three roles and wires staffing:

| Dev Type | Typical harness (operator-chosen) | Mission Types / duties |
|---|---|---|
| `judge` | operator pick (often a judgment-heavy harness) | ONBOARD, PLAN, REVIEW |
| `executor` | operator pick (often an execution harness) | EXECUTE |
| `steward` | operator pick | Steward duties — relations + discovery routing (EXECUTE-grade bar, ADR-0033 D10) |

The wizard also pins each role's CLI to a concrete semver (`resolve_latest`, falling back to `HOUSE_PINS` offline — never the literal string `latest`), sets `steward.dev_type = steward`, and seeds Development + Customer Success identifying-prompt templates under the new names. **Existing non-empty rosters never see the wizard** and are never renamed or migrated (assignments, breakers, receipts, and `/data/secrets/{name}/` all key on the name).
...
The Mission-Type→Dev-Type assignment lives in `AppConfig.assignments` (§9). Fresh installs start with an **empty** (unstaffed) map; after first-setup (or a manual edit) each Mission Type maps to exactly one Dev Type, and a Dev Type may serve any number of Mission Types. Deleting the `assignments:` key restores the unstaffed empty map. A PMO instance may **override** rows for itself (`PMOInstance.assignments`, ADR-0019): a present key replaces the global row wholesale (args included), an absent key inherits it live — so the one-type-one-Dev rule holds *per instance*, and different boards can staff different crews on one deployment.
...
| `outcome_summary` | `str` (default `""`) | Compact UI line stamped at steward finalize from apply counts (e.g. `"3 relations proposed (1 rejected)"` / `"5 discoveries shared (0 rejected)"`). Empty on non-steward / legacy / failed-before-apply. Exposed on the runs list/detail allowlist for synthetic-key hover (CAKE-167); never a stand-in for serializing `result`. |
...
| `delivery_to` | `str` (default `""`) | The change set's **delivery destination** this run was dispatched under — `markers.delivery_of(live.description)` at dispatch: `"repository"` or `"ticket"` (`""` on legacy records reads as repository). Snapshotted like `stage_label_at_dispatch`: a person editing the description marker mid-run steers the *next* dispatch, never re-judges the running one, and finalize-time readers pay no PMO read (`03-mission-lifecycle.md` §1.2, ADR-0017 addendum). |
...
| `pmos` | `list[PMOInstance]` — `{name, system, team_key, api_base, repos, reference_repos, memory_repos, assignments, active_prompt_templates, …}` | Instance `name` is the identity (`^[a-z][a-z0-9_]{0,38}$` — GitHub-aligned subset; underscore OK, no hyphen/dot; CAKE-151). Uppercased it prefixes branches in full; run ids use a 12-char scrubbed-upper prefix with config-time collision refusal across PMO instances (only PMOs mint run ids — a repo or skill source may share a PMO's name, the Curator-board shape). `system` validated against `PMO_SYSTEMS`. **`api_key` is a read-through** over the GUI secret store (`/data/secrets/connections/pmo-{name}.json`) — not an env-var name. `repos` is the ordered work-repo set for that instance (first = default for unmarked missions); `reference_repos` are read-only consultation clones, disjoint from the work set. `memory_repos` are board-bound notebooks (PLAN_MEMORY): cloned to `/workspace/memory/<card>/`, pairwise disjoint from `repos` and `reference_repos` (I1). A memory-bound card may be a work repo only on a Curator board whose `repos == [that card]` (I2). `assignments` is the instance's Mission-Type override map (ADR-0019): present key = wholesale override of the global row below, absent = inherit live; empty `dev_type` and unknown mission-type keys refused. `active_prompt_templates` is the instance's Mission-Type → playbook template override map (ADR-0037 / CAKE-150): present key = that template name for this instance only, absent = inherit the global `active_prompt_templates` below live; blank values and unknown mission-type keys refused. |
| `repos` | `list[RepoInstance]` — `{name, forge, url, api_base, default_branch, auto_merge, auto_resolve_merge_conflicts, merge_settle_minutes, merge_retry_window_minutes, …}` | `forge` validated against `forges()`. **`default_branch` blank (the default) = the repository's own default, inquired from its HEAD before every mirror sync**; a value is a deliberate pin, verified at every sync — a pin the repository lacks fails that card's sync loud (missions on it defer with the reason) instead of mounting an empty tree. Everything that needs a name (the Dev's env and playbook, branch protection) reads the **resolved** branch, never this raw field; the claims writer clones a blank card at the repository's own HEAD. An empty repository (no branches yet) bootstraps a blank card on `main`. The card's **Discover default branch** action fills the field with what the repository names right now. **`token` / `token_ro` / `reviewer_token` are read-throughs** over `/data/secrets/connections/repo-{name}.json` — not `*_env` fields. **Merge doctrine is per repo** (ADR-0020): `auto_merge` (default `false`) — when true, the **app** merges this repo's PRs after REVIEW approval; when false, parks at `DEVCAKE-MERGE` (does **not** strip Dev forge-token merge capability — that is branch protection, `14-security.md` §2 zone C). `auto_resolve_merge_conflicts` (default `true`, inert while `auto_merge` off) — on conflict/stale branch, route back to EXECUTE (max 2 attempts). `merge_settle_minutes` (default 0, `≥ 0`, inert while `auto_merge` off) — post-approve coalesce wait before first auto-merge; end-of-window freshness recheck may re-open REVIEW. `merge_retry_window_minutes` (default 30, `≥ 0`, inert while `auto_merge` off) — deferred-merge sweep window (forge readiness). **Internal (zero-repo) synthesized instances always set `auto_merge=True`** at provision time — the zip deliverable depends on merge; operators who want doctrine control create the repo as a config card instead. **`internal`** (runtime-only — a private attribute, not a field: never parsed from input, never in a dump, a schema, or the SPA card scaffold) marks such a synthesized row, which stores **no** connection secrets: its `token` / `token_ro` / `reviewer_token` read-throughs answer `""` instead of consulting the secrets store, whose instance-name check rejects the hyphenated internal name. |
...
| ~~`attach_merged_changeset_to_pmo`~~ | *retired* | Gone (ADR-0017 addendum). The ticket receives a change set exactly when the mission's recorded **delivery destination** is the ticket (`03-mission-lifecycle.md` §1.2, §4), or — as always — after merge for the invisible internal forge. Pre-v1: an old key is an unknown key (generic warning, no migration). Was: | When true, after a REVIEW-approved **merge** the app also zips the PR change set onto the PMO feed for **configured** work repos. Internal/zero-repo missions always attach a zip (ADR-0010) regardless. Leave off for eng repos — the forge PR is canonical; the zip is a merge-time snapshot (size-capped, may omit files). Best-effort: packaging failure never un-Dones the mission. |
...
| `pmos[].plan_approval` | `bool` (default `false`) | Per-board plan approval (`03-mission-lifecycle.md` §2a): while true, every fresh plan — a PLAN run's or one attached at ONBOARD triage — lands on `DEVCAKE-EXECUTE` **and** `DEVCAKE-NEEDS-HUMAN`, and every decomposition child is created parked under `DEVCAKE-NEEDS-HUMAN` (a split is a plan), so nothing is scheduled until a person approves by removing the label (or **Resume** in the admin panel), one ticket at a time. A board's process policy, so it lives on the PMO instance, not the repo card. Not a hand-off: never counted by the loop guardrail. Drafted on the PMO page, applied on Save. |
| `pmos[].managed` | `bool` (default `false`) | ADR-0030 / CAKE-157: app-managed instance (the auto-provisioned default board). The **flag** is the durable identity — default insert name is `board`, but the display name is operator-owned (ordinary PMO rename on Save). `system`/`team_key`/`api_base`/`managed` are canonicalized and the row survives wholesale `pmos` replaces via `reconcile_managed_pmos` while the bundled provisioner exists; `name`/`repos`/`reference_repos`/`assignments`/`active_prompt_templates`/`intake_paused` stay operator-owned. Operators cannot mark arbitrary rows managed (stray flags are stripped). |
...
| `stall_after_seconds` | `int` (default `1800`) | A dispatch refused or deferred for longer than this, for a reason a person can act on (never a dependency wait), is a **stall**: listed in `/health.stalled_dispatches`, raised on the admin Overview, badged on the board, and surfaced on the ticket once per block per episode (ADR-0042 addendum, `stalls.py`). |
...
| `skill_sources` | `list[SkillSource]` — `{name, forge, url, default_branch, subdir, backed_by}` (default `[]`) | ADR-0016 addendum 2 (2026-08-14): dedicated skills connections — NEVER repo cards. `name` is instance-shaped and shares the mirror namespace with `repos` (collisions refused). `subdir` is a relative path inside the repository holding the `<skill>/SKILL.md` dirs (`""` = root; `..` refused). Read tokens are GUI-stored under the `skill:` secret scope (`token_ro` preferred, `token` fallback — read-through properties, never serialized); a source has no reviewer token and no PR surface. Always mirror-eligible (synced with the forge DESCRIPTOR's clone_user — no live adapter is built, no health probing); read-only by construction; no PMO can select one. Managed on the Skills page. Dev Types reference its skills as `<name>/<skill>`. **`backed_by` (ADR-0039)** names a repository card whose mirror serves this source's reads instead of a URL of its own — one physical mirror, that card's token and sync; mutually exclusive with `url`, must cite a CONFIGURED repo card (which also refuses deleting or un-configuring the backing card while cited), citation follows a repo rename, and flipping an own-remote source to backed deletes its old mirror; `default_branch`/`subdir` keep their meaning against the shared mirror — an existing pin is served and probed (missing pin fails loud), an empty branch means the backing card's branch. |
| `cost_inputs` | `{rates: [{model_prefix, input_per_mtok, cache_read_per_mtok, cache_write_per_mtok, output_per_mtok}], override_native: bool}` (default: empty rates / `false`) | Operator rate card behind app-side cost **estimates** (`adr/0021`). Ships empty (CAKE-174) — operators enter USD per 1M tokens from their model vendor's published API pricing page; matched by longest `model_prefix`; unknown models are never priced. `override_native` flips DISPLAY surfaces to prefer the rate-card computation over harness-reported cost. The derived `rate_card_id` (`builtin-v3` when rates match the shipped empty card, else `operator:<hash8>`) labels every stamped estimate (`config.BUILTIN_RATE_CARD_ID`). |
| `active_prompt_templates` | `dict[str, str]` (default `{}`) | **Global** per-Mission-Type active prompt template name; missing key ⇒ built-in Development (legacy alias `"default"`). Per-instance overrides on `pmos[*].active_prompt_templates` take precedence when present (ADR-0037); resolution is `active_prompt_template_for(config, instance, mission_type)` — the only read path dispatch and health use. |
...
| `rate_card_id` | `str \| None` | App-stamped alongside the estimate: `builtin-v3` (shipped empty defaults) or `operator:<hash8>` (edited card). Present iff the estimate is. |
...
| `repo` | `str` | Optional. The card name (or the repository URL's last path segment) of the repository this child's work lands in. Validated by the app against the instance's **work** repositories (a reference repo or an unknown name rejects the manifest as `DEV_BAD_OUTPUT`); the app stamps the child's `` `devcake-repo:<card>` `` footer itself. Precedence: the draft's `repo`, else the parent's marker, else the instance default. A marker written inside the draft's *description* never routes — Dev-written marker syntax is neutralized on every append path. |
...
| `delivery_to` | `"repository" \| "ticket"` | Optional. The child's **delivery destination** from birth (ADR-0017 addendum): `ticket` means the child's change set is attached to its ticket at REVIEW approve and its pull request closed unmerged; absent or `repository` means merge as usual. The app stamps the child's `` `devcake:delivery:v1 to=ticket` `` footer line itself (never Dev prose); on a board that cannot receive files the child stays on the repository and the audit says so. The child's own ONBOARD may only *propose* a change afterwards. |
| `delivery_reason` | `str` | Required with `delivery_to: "ticket"`: one line saying why no repository should change — redacted, marker-defanged and capped before it lands under the footer marker. A `ticket` draft without it rejects the manifest as `DEV_BAD_OUTPUT`. |

`parent_ref` is **not** part of the draft — the app itself passes `create_mission`'s `parent_ref`: for a decomposed **Project**, its own `pmo_id` (children land inside it); for a decomposed **Issue**, the issue's containing project when it has one (`adr/0012` — the family stays in the project, and the tracking sweep waits for the grandchildren). The app adds the `DEVCAKE-CREATED` label (plus `DEVCAKE` in opt-in mode) on creation; the Dev does not manage labels (INV-4). The machine marker in each child's footer — `devcake:decomposition:v1 parent={pmo_id} manifest={sha256} part={i}/{n} depth={d}` — is the PMO-held record of lineage **and** generation: `depth` is optional in the parse (a marker without it is a level-1 child from the depth-1 era), and the canceled original carries the reverse pointer as a description note (`_Decomposed by DevCake into {children keys}_`, appended via `PMOPort.append_description` — through the one description-append path, `feed.append_note`, like every marked note). A child drafted with `delivery_to: "ticket"` also carries `` `devcake:delivery:v1 to=ticket` `` and its reason line in that footer (ADR-0017 addendum).
