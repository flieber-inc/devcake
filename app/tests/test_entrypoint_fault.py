...
# ── CAKE-82: exit-11 forensics always carry `terminal` ───────────────────────

def test_every_exit_11_fail_assigns_forensics_terminal():
    """docs/07 §4: every exit-11 artifact's evidence includes key `terminal`
    (value may be null). Plan-mode empty-plan must match the other exit-11 arms."""
    import ast
    ...
    def is_fail11(stmt):
        if not isinstance(stmt, ast.Expr) or not isinstance(stmt.value, ast.Call):
            return False
        call = stmt.value
        if not isinstance(call.func, ast.Name) or call.func.id != "fail":
            return False
        if not call.args or not isinstance(call.args[0], ast.Constant):
            return False
        return call.args[0].value == 11

    def sets_terminal(stmt):
        if not isinstance(stmt, ast.Assign) or len(stmt.targets) != 1:
            return False
        t = stmt.targets[0]
        return (isinstance(t, ast.Subscript)
                and isinstance(t.value, ast.Name) and t.value.id == "forensics"
                and isinstance(t.slice, ast.Constant)
                and t.slice.value == "terminal")

    missing = []

    def check_body(body, where):
        armed = False
        for stmt in body:
            if sets_terminal(stmt):
                armed = True
            if is_fail11(stmt):
                if not armed:
                    missing.append(f"{where}:line {stmt.lineno}")
                armed = False
            for attr in ("body", "orelse", "finalbody"):
                nested = getattr(stmt, attr, None)
                if isinstance(nested, list):
                    check_body(nested, f"{where}/{type(stmt).__name__}@{stmt.lineno}")
            if isinstance(stmt, ast.Try):
                for h in stmt.handlers:
                    check_body(h.body, f"{where}/handler@{h.lineno}")

    for node in tree.body:
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            check_body(node.body, node.name)
    assert not missing, (
        "exit-11 fail() without forensics['terminal'] beforehand "
        f"(docs/07 §4): {missing}")


def test_no_claude_code_third_level_harness_default():
    """docs/07 §3 + module doctrine: DEVCAKE_HARNESS is runspec → image ENV
    only. A missing/empty value must not silently become 'claude-code'."""
    src = ENTRYPOINT.read_text()
    assert 'get("DEVCAKE_HARNESS", "claude-code")' not in src
    assert "get('DEVCAKE_HARNESS', 'claude-code')" not in src
