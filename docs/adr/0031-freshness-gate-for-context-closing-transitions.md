...
| Auto-merge post-approve settle → first merge attempt (`merge_settle_minutes` > 0) | yes, app-driven | **full recheck at window end** (`freshness.recheck_and_maybe_rereview`): unread material → 🔄 re-review + `DEVCAKE-REVIEW`; clean/exhausted → proceed to merge-retry / merge. Coalesces sibling `discovery-in` without per-post thrash |
| Operator Force re-check on `DEVCAKE-MERGE` (`force_freshness`) | yes, operator-initiated | **full recheck on demand** (same helper): material → re-open REVIEW; nothing unread → no-op. Admin Missions ⋯ |
| REVIEW approve → `DEVCAKE-MERGE` (human merges) → done | yes, but human-attended | exempt from *automatic* re-review: a person is at the merge button with the live feed in front of them; use Force when machine re-judgment is wanted |
