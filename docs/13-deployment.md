...
the profile); the baker notices within a minute and every surface says so. Rolling Dagu back = the previous release checkout
...
before bringing the old Dagu up — with the stack down, using the same
digest-pinned helper image the backup scripts use (`DEVCAKE_BACKUP_IMAGE`,
default `debian:bookworm-slim@sha256:88200866dfff7ea7f5cbcb6ec7c8a701889efe6fe859fe64d6990e4b07ea4171`):
...
aside if you expect to roll back later — the newest three are kept.
