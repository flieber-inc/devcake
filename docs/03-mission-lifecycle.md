...
The Dev must: check out the PR branch in its clone; diff against the plan; hunt for bugs, flaws, and omissions; run the tests if present. The playbook prompt forbids rubber-stamping — the default posture is distrust. The same never-merge / never-forge-approve / never-push-default hard rule as EXECUTE applies: the REVIEW verdict lives in `result.json` only; formal forge approval and merge are app side effects (or a human) — lifecycle gates and receipts live above the Dev.
